    ...
    removeAttribute(attr: Attribute): void {
        ...
        if (idx !== -1) {
            ...
            for (let i = 0; i < this._attributes.length; i++) {
                if (this._attributes[i].getLocalName() === attr.getLocalName() &&
        ...
        try {
            ...
            if (Array.isArray(result)) {
                for (const node of result) {
                    ...
                    } else if (domNode.nodeType === 2) { // ATTRIBUTE_NODE
                        const ownerElem = (domNode as globalThis.Attr).ownerElement;
                        if (ownerElem) {
                            const mappedElem = this.findCorrespondingElement(ownerElem);
                            if (mappedElem) {
                                const attr = mappedElem.getAttribute(domNode.localName || domNode.nodeName, domNode.namespaceURI || undefined);
...
export class Builder {
    build(xml: string): Document {
        ...
        if (errorNode.length > 0) {
            throw new ParsingException('XML parsing error: ' + (errorNode[0].textContent || 'Unknown error'));
        }
