 ...
 * the face of MEI's inconsistent namespace usage. Everything here matches on `localName` and
 ...
 * `getFirstChildElement(Element)` and `getFirstChildElement(String, Element)`, and the port
 * added `(Element, String)` on top. TypeScript dispatches them at runtime by inspecting
 * `typeof arg1`, which is why the implementation signature is widened to
 * `Element | string | null`.
 *
 * The two element-first forms differ only by a trailing argument and are therefore one
 * signature with an optional parameter. What stays split is name-first against name-last,
 * because those select *different implementations* (see below).
 */
...
/**
 * this method is an alternative to XOM's getChildElements(String name) which sometimes
 * doesn't seem to work
 *
 * Always returns an array — empty when nothing matches. Java's two null guards (`ofThis ==
 * null`, `name === ''`) are gone under ARCHITECTURE.md RULE N2b: `parent` is non-nullable,
 * and every call site passes either a string literal or no name at all.
 *
...
export function allChildElements(parent: Element, name?: string): Element[] {
  // `getChildElements` already selects element children by local name, in document order, and
  // returns the live nodes; the `query("child::*[…]")` this replaces serialised the whole
  // subtree, re-parsed it and mapped every hit back by position (see {@link Element.query}).
  // The `toArray()` copy is not optional: the return type is a mutable `Element[]` and
  // callers splice it.
 ...
 *
 * This is the `descendant::*[…]` axis, written out — cheaper than {@link Element.query},
 ...
 * `dateMap.addToMap`, which looks for the last entry not later than the one being inserted,
 * and the converter's tie handling, which looks for the note a tie continues. Building the
 * whole array for that is a linear pass per insertion, so filling a score of n notes was
 * Θ(n²). Here the walk is lazy: it descends the rightmost spine, yields, and only goes
 * further if the caller keeps asking, so for a map whose entries are childless and whose
 * dates arrive in order the caller stops after one element and the call is O(depth).
 *
...
export function* reverseDescendantElements(
  ...
  // `while ((frame = stack.pop()) !== undefined)`. Taking the peek in the for-header is what
  // lets the compiler see that the body runs only on a frame that exists; `continue` runs the
  // update expression, so the re-peek happens on every path.
...
/**
 * Create a flat list of all descendants with a certain attribute (beginning with ofThis), in
 * document order. Null when there is nothing to search, as {@link getAllDescendantsByName}.
 */
 ...
 * clones does. (The same divergence is documented at `Msm.cloneElement`.)
 *
 * The `null` in the return type comes only from the `null` in the argument type, and the two
 * overloads say so — `cloneElement(someElement)` is an `Element`. That is RULE N2b's
...
/**
 * returns the attribute with the specified name contained in ofThis, or null if that
 * attribute does not exist, namespace is ignored
 *
...
export function attribute(name: string, ofThis: Element | null): Attribute | null {
  ...
  // Java's three lookups all miss and `Helper.getAttributeValue` answers `""`, so step one has
  // to miss here too. Two call sites depend on it and both are byte-visible: `Msm.ts`'s
  ...
  // of length zero.
  //
  // The fidelity belongs here rather than in `Element.getAttribute`, which keeps its
  // convenience for everyone else: this function is the transcription of
  // `Helper.getAttribute`. Removing the qualified match there instead also passes the gate,
  // and reds 30 tests elsewhere.
  //
  // Consequence: step one matches a local name in NO namespace and steps two and three do not
  // cover the gap, so an attribute in a namespace that is neither empty nor the element's own
  // is not found here at all. That is unreachable from any document in this repository —
  // `xml:id` is the only namespaced attribute in the
...
/**
 * returns the value of attribute name in Element ofThis as String, or empty string if the
 * attribute does not exist, namespace is ignored
 */
...
/**
 * The throwing sibling of {@link getAttributeValue} (ARCHITECTURE.md RULE N2a).
 *
 ...
 * empty" cannot use it. This is `element.getAttributeValue(name)` (XOM's own accessor, which
 * returns `null`) with the caller's assertion replaced by an error that names the attribute.
 *
 * Exactly equivalent to `ofThis.getAttributeValue(name)!`, which is not obvious, because
 ...
 * `getLocalName() === name` (or the qualified name), so if it finds nothing then no attribute
 * carries that local name and neither namespaced retry can find one either. The fallbacks
 * earn their keep against Java XOM, whose one-argument `getAttribute` matches unnamespaced
 * attributes only, and are kept for that documentary value rather than deleted under RULE N2b.
 */
export function requireAttributeValue(name: string, ofThis: Element | null): string {
  return requireAttribute(name, ofThis).getValue();
}

/** returns the parent element of ofThis as element, or null when the parent is not an element */
