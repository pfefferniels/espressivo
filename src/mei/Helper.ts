import { Attribute, Builder, Document, Element, Elements, Nodes, XomNode } from '../xml/XomTypes.js';
...
export class Helper {
    ...
    static validateAgainstSchema(file: string, schema: string): void {
        console.warn("validateAgainstSchema: Schema validation is not available in the browser/Node.js environment. Skipping validation.");
    }
    ...
    static getFirstChildElement(arg1: Element | string | null, arg2?: Element | string | null): Element | null {
    ...
    static getAllChildElements(arg1: string | Element | null, arg2?: Element | null): Element[] | null {
        if (arg1 === null || arg1 === undefined) return null;
     ...
     * @return
     */
    static getAllDescendantsWithAttribute(attrName: string, ofThis: Element | null): Element[] | null {
    ...
    static getNextSiblingElement(arg1: Element | string | null, arg2?: Element | null): Element | null {
    ...
    static getPreviousSiblingElement(arg1: Element | string | null, arg2?: Element | null): Element | null {
        if (arg1 == null) return null;
    ...
    static addToMap(addThis: Element | null, map: Element | null): number {
        ...
        for (let i = es.size() - 1; i >= 0; --i) {                             // go through the elements
            if (parseFloat((es.get(i) as Element).getAttributeValue("date")!) <= date) {  // if the element directly before date is found
    ...
    static extractAllIntegersFromString(input: string): number[] {
        let str = input.replace(/ bis /g, " -").replace(/ to /g, " -");
        const p = /-?\d+/g;
     ...
     */
    static updateMpmNoteidsAfterResolvingRepetitions(map: { getXml(): Element }, noteIdMappings: Map<string, string>): void {
        for (const [key, _value] of noteIdMappings) {                                              // for all mappings
            ...
            for (let i = 1; i < ns.size(); ++i) {                               // iterate through the elements starting with the second
    ...
    static decimalDuration2HtmlUnicode(duration: number, isRest: boolean): string {
        ...
        if (duration < 0.015625)
            return (isRest ? "&#119106;" : "&#119140;") + Helper.durationRemainder2UnicodeDots(0.0078125, duration - 0.0078125);
        if (duration < 0.03125)
            return (isRest ? "&#119105;" : "&#119139;") + Helper.durationRemainder2UnicodeDots(0.015625, duration - 0.015625);
        if (duration < 0.0625)
            return (isRest ? "&#119104;" : "&#119138;") + Helper.durationRemainder2UnicodeDots(0.03125, duration - 0.03125);
        if (duration < 0.125)
            return (isRest ? "&#119103;" : "&#119137;") + Helper.durationRemainder2UnicodeDots(0.0625, duration - 0.0625);
        if (duration < 0.25)
            return (isRest ? "&#119102;" : "&#119136;") + Helper.durationRemainder2UnicodeDots(0.125, duration - 0.125);
        if (duration < 0.5)
            return (isRest ? "&#119101;" : "&#119135;") + Helper.durationRemainder2UnicodeDots(0.25, duration - 0.25);
        if (duration < 1.0)
            return (isRest ? "&#119100;" : "&#119134;") + Helper.durationRemainder2UnicodeDots(0.5, duration - 0.5);
        if (duration < 2.0)
            return (isRest ? "&#119099;" : "&#119133;") + Helper.durationRemainder2UnicodeDots(1.0, duration - 1.0);
        if (duration < 4.0)
            return (isRest ? "2 &#119098;" : "&#119132;") + Helper.durationRemainder2UnicodeDots(2.0, duration - 2.0);
        if (duration < 8.0)
            return (isRest ? "4 &#119098;" : "&#119223;") + Helper.durationRemainder2UnicodeDots(4.0, duration - 4.0);
        if (duration === 8.0)
            return isRest ? "8 &#119098;" : "&#119222;";
        else
            return isRest ? "rest" : "note";
     ...
     * @return
     */
    private static durationRemainder2UnicodeDots(undottedNoteValue: number, remainder: number): string {
        let dots = "";
     ...
     */
    static midi2PnameAndAccid(useSharpInsteadOfFlat: boolean, midipitch: number, pnameAccid: string[]): void {
        if (pnameAccid.length < 2) {
            console.error("Error in method Helper.midi2PnameAndAccid: Array length of pnameAccid should be at least 2.");
            return;
        }
     ...
     */
    static midi2PnameAccidOct(useSharpInsteadOfFlat: boolean, midipitch: number, pnameAccidOct: string[]): void {
        if (pnameAccidOct.length < 3) {
            console.error("Error in method Helper.midi2PnameAccidOct: Array length of pnameAccidOct should be at least 3.");
            return;
        }
    ...
    static xslTransformToDocument(input: Document, xsltOrTransformer: string | any): Document | null {
        console.warn("xslTransformToDocument: XSLT transforms are not available in the browser/Node.js environment. Returning null.");
        return null;
    }
    ...
    static xslTransformToString(input: Document | string, xsltOrTransformer: string | any): string | null {
        console.warn("xslTransformToString: XSLT transforms are not available in the browser/Node.js environment. Returning null.");
        return null;
    }
    ...
    static makeXsltTransformer(xslt: string, processor: any, source: any, destination: any): any {
        console.warn("makeXsltTransformer: XSLT is not available in the browser/Node.js environment. Returning null.");
        return null;
    }
    ...
    static makeXslt30Transformer(xslt: string, processor?: any): any {
        console.warn("makeXslt30Transformer: XSLT is not available in the browser/Node.js environment. Returning null.");
        return null;
    }
     ...
     */
    static addToListAttribute(element: Element | null, attrName: string | null, value: string | null): void {
        ...
        const values = currentValue.trim().split(/\s+/).filter(s => s !== '');
