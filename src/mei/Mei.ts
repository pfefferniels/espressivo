...
export class Mei extends XmlBase {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    return (this.getFile() === null) ? "" : Helper.getFilenameWithoutExtension(Mei.fileBasename(this.getFile()!));
                }
            }
        }
        return (title !== null) ? title.getValue() : ((this.getFile() === null) ? "" : Helper.getFilenameWithoutExtension(Mei.fileBasename(this.getFile()!)));
    }
    ...
    getAllMdivs(): Element[] {
        ...
        if (music !== null)
            result.push(...this._getAllMdivs(music));
        return result;
    }
        ...
        for (let i = 0; i < children.size(); i++) {
            ...
            switch (e.getLocalName()) {
                ...
                case "mdiv": {
                    ...
                    if (subMdivs.length === 0)
                        result.push(e);
                    else
                        result.push(...subMdivs);
                    break;
                }
            }
        }
    ...
    exportMsm(ppq?: number, dontUseChannel10?: boolean, ignoreExpansions?: boolean, cleanup?: boolean): Msm[] {
        return this.exportMsmMpm(ppq, dontUseChannel10, ignoreExpansions, cleanup).getKey();
    }

    exportMsmMpm(ppq: number = 720, dontUseChannel10: boolean = true, ignoreExpansions: boolean = false, cleanup: boolean = true): KeyValue<Msm[], Mpm[]> {
    ...
    exportMusicXml(ignoreExpansions: boolean = false): MusicXml[] {
    ...
    computeMinimalPPQ(): number {
        ...
        for (let i = durs.size() - 1; i >= 0; --i) {
            ...
            let d = (elem.getAttribute("dur") !== null) ? Helper.duration2decimal(elem.getAttributeValue("dur")!) : 4.0;
    ...
    resolveCopyofs(): string[] | null {
        ...
        while (true) {
            ...
            const all = e.query("descendant::*[attribute::copyof or attribute::sameas or attribute::xml:id]");
    ...
    addIds(): number {
        ...
        const e = root.query("descendant::*[(local-name()='measure' or local-name()='note' or local-name()='rest' or local-name()='mRest' or local-name()='multiRest' or local-name()='chord' or local-name()='tuplet' or local-name()='mdiv' or local-name()='reh' or local-name()='section') and not(@xml:id)]");
