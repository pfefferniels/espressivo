...
export class Audio {
  ...
  exportWaveformImage(width: number, height: number): Uint8ClampedArray | null {
    ...
    for (let chanIdx = 0; chanIdx < channels.length; ++chanIdx) {
      ...
      const waveformData = Audio.computeWaveformData(
        channel,
        0,
        channel.length - 1,
        width,
   ...
   */
  static computeWaveformData(
    audio: number[],
    leftmostSample: number,
    rightmostSample: number,
    width: number,
    height: number,
  ): Uint8Array | null {
    ...
    for (let i = 0; i < sampleCount; ++i) {
      ...
      if (maxValues[x][0] < audio[sampleIndex]) maxValues[x][0] = audio[sampleIndex];
      if (maxValues[x][1] > audio[sampleIndex]) maxValues[x][1] = audio[sampleIndex];
    }
