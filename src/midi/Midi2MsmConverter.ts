...
export class Midi2MsmConverter {
  ...
  private channel = 0; // this indicates the current midi channel that subsequent (meta) events are sent to
  private port = 0; // this represents the current midi port that subsequent events are sent to
  ...
  private useSharpsInsteadOfFlats = true; // this is needed for encoding accidentals
   ...
   */
  constructor(
    midiFileFormat: number,
    useDefaultInstrumentNames: boolean,
    sequence: Sequence,
    msm: MsmLike,
    helper: HelperLike,
  ) {
    ...
    console.log(
      `Converting noteOn (with velocity 0) to noteOff events: ${Midi.noteOns2NoteOffs(this.sequence)} events converted.`,
    );
  ...
  convert(): void {
    ...
    for (const track of this.tracks) {
      ...
      for (let e = 0; e < track.size(); ++e) {
        // for all the events in the track
        const event = track.get(e); // get the current event
        if (this.processShortEvent(event))
          // try processing it as short event
          continue;
        if (this.processMetaEvent(event))
          // try processing it as meta event
          continue;
        if (this.processSysexEvent(event))
          // try processing it as sysex event
          continue;
        console.error(
          `Unknown MIDI message: ${event.getMessage().constructor.name} at timecode ${event.getTick()}.`,
        ); // I have no idea what kind of event/message this could be
      }
  ...
  private processMetaEvent(event: MidiEvent): boolean {
    ...
    if (!(msg instanceof MetaMessage)) return false;
    ...
    switch (m.getType()) {
      ...
      case EventMaker.META_Track_Name: {
        // these events can occur before we are in a specific channel/part
        const textDecoder = new TextDecoder();
        this.trackname +=
          (this.trackname.length === 0 ? '' : ' - ') + textDecoder.decode(m.getData()).trim(); // store the track name
        if (this.currentPart !== this.global) {
          // if we are already local
      ...
      case EventMaker.META_Instrument_Name: {
        if (this.currentPart !== this.global) {
          // it makes no sense to give the global environment a track or instrument name
          let name: string;
          const textDecoder = new TextDecoder();
          const instName = textDecoder.decode(m.getData()).trim();
          const nameAtt = this.currentPart.getAttribute('name')!;
          const namePart = nameAtt.getValue();
          if (namePart.length === 0) name = instName;
          else if (namePart === this.trackname) name = this.trackname + instName;
          else name = `${namePart} - ${instName}`;
      ...
      case EventMaker.META_Marker: {
        ...
        const markerMap = this.currentPart
      ...
      case EventMaker.META_Midi_Channel_Prefix: {
        // all meta messages that follow go to this channel
      ...
      case EventMaker.META_Midi_Port: {
        // all messages that follow go to this port
      ...
      case EventMaker.META_Time_Signature: {
        // decoding time signature messages
      ...
      case EventMaker.META_Key_Signature: {
        // decode key signature and make an msm representation of it
        ...
        for (const accidental of accidentals) ks.appendChild(accidental);

        // add the key signature to the key signature map
        const ksMap = this.currentPart
          .getFirstChildElement('dated')!
    ...
    switch (m.getCommand()) {
      case EventMaker.NOTE_OFF: {
        ...
        for (let i = 0; i < this.pendingNotes.length; ++i) {
          // search the pendingNotes list for the first note with the same pitch
      ...
      case EventMaker.NOTE_ON: {
        ...
        this.helper.addToMap(
          note,
          part.getFirstChildElement('dated')!.getFirstChildElement('score')!,
        );
      ...
      case EventMaker.PROGRAM_CHANGE: {
        ...
        if (this.trackname.length === 0) {
          const instName = InstrumentsDictionary.getInstrumentName(
            m.getData1(),
            this.useDefaultInstrumentNames,
          );
          if (nameAtt.getValue() === this.trackname) {
            // if the part has no name or just the track name
            nameAtt.setValue((this.trackname.length === 0 ? '' : `${this.trackname}: `) + instName); // set the name
          } else {
            // if there is already more than the track name
  ...
  private processSysexEvent(event: MidiEvent): boolean {
    const msg = event.getMessage();
    if (!(msg instanceof SysexMessage)) return false;
