...
export class Midi {
    ...
    append(midi: Midi): void {
        ...
        for (let t = 0; t < clone.getSequence().getTracks().length; ++t) {     // go through all tracks of the sequence to be added
    ...
    convertPPQ(ppq: number): void {
        ...
        console.log("Converting timing basis of \"" + (this.getFile() || "unnamed") + "\" from " + ppqOld + " to " + ppq + " pulses per quarter note.");

        const newSequence = new Sequence(Sequence.PPQ, ppq);     // create a new MIDI sequence
        for (const track of this.sequence!.getTracks()) {        // for each track in the old sequence
            const newTrack = newSequence.createTrack();           // create a new track in the new sequence
            for (let e = 0; e < track.size(); ++e) {              // for each MIDI event in the old track
    ...
    static getMinimalPPQ(sequence: Sequence, onlyNotes: boolean): number {
        if (sequence === null)
            throw new Error("Error: MIDI sequence is null.");
     ...
     */
    getTempoData(): Array<{ tick: number; bpm: number; beatlength: number }> {
        const tempoData: Array<{ tick: number; bpm: number; beatlength: number }> = [];
    ...
    static noteOns2NoteOffs(sequence: Sequence): number {
        let eventsChanged = 0;
        for (let t = 0; t < sequence.getTracks().length; ++t) {                                     // for all tracks
            for (let e = 0; e < sequence.getTracks()[t].size(); ++e) {                              // for all events in the track
                const msg = sequence.getTracks()[t].get(e).getMessage();
                if (msg instanceof ShortMessage) {                                                   // if it is a ShortMessage
                    const sm = msg;
                    if ((sm.getCommand() === ShortMessage.NOTE_ON) && (sm.getData2() === 0)) {        // if this is a noteOn with velocity 0
     ...
     */
    exportMsm(useDefaultInstrumentNames: boolean = true, cleanup: boolean = true): any /* Msm */ | null {
