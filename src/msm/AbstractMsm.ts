...
export abstract class AbstractMsm extends XmlBase {
     ...
     */
    protected static makePartFromString(name: string, number: string, midiChannel: number, midiPort: number): Element {
    ...
    static getElementAtAfterByName(name: string, date: number, map: Element): Element | null {
        let es: Elements;
        if (name.length === 0)              // if no specific name given
    ...
    deleteEmptyMaps(): void {
        ...
        const maps: Nodes = this.getRootElement()!.query("descendant::*[contains(local-name(), 'Map')]");   // get all elements in the document that have a substring "Map" in their local-name
        for (let i = 0; i < maps.size(); ++i) {                                          // go through all these elements
