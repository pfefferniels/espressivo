...
export class Msm extends AbstractMsm {
    ...
    static createMsm(title: string, id: string | null, ppq: number): Msm {
        ...
        const idAttribute = new Attribute("xml:id", "http://www.w3.org/XML/1998/namespace", (id === null) ? uuidv4() : id);  // make new id attribute
    ...
    convertPPQ(ppq: number): void {
        ...
        console.log("Converting timing basis of \"" + this.getTitle() + "\" from " + this.getPulsesPerQuarter() + " to " + ppq + " pulses per quarter note.");

        this.setPPQ(ppq);

        // find all attributes date, date.end and duration, and convert their values
        const atts: Nodes = this.getRootElement()!.query("descendant::*[attribute::date]/attribute::date | descendant::*[attribute::date.end]/attribute::date.end | descendant::*[attribute::duration]/attribute::duration");
    ...
    getMinimalPPQ(): number {
        ...
        for (const part of parts) {                                                                                             // go through all parts
            ...
            for (let j = 0; j < notes.size(); ++j) {                                                                           // go through all notes
     ...
     */
    static makePartFromString(name: string, number: string, midiChannel: number, midiPort: number): Element {
     ...
     * @returns the part element just generated
     */
    static override makePart(name: string, number: number, midiChannel: number, midiPort: number): Element {
    ...
    getPart(number: number, name: string, midiChannel: number, midiPort: number): Element | null {
        ...
        for (const part of parts) {
            ...
            if ((numberAtt !== null) && (parseInt(numberAtt.getValue()) === number))
                return part;
        }
        ...
        for (const part of parts) {
            ...
            if ((nameAtt !== null) && (nameAtt.getValue() === name))
                return part;
        }
        ...
        for (const part of parts) {
            ...
            if ((portAtt !== null) && (parseInt(portAtt.getValue()) === midiPort)) {
                ...
                if ((channelAtt !== null) && (parseInt(channelAtt.getValue()) === midiChannel))
                    return part;
            }
        }
     ...
     * @returns
     */
    static makeTimeSignature(date: number, numerator: number, denominator: number, id: string | null): Element {
    ...
    resolveSequencingMaps(): Map<string, string> {
        ...
        const globalSequencingMap = this.getRootElement()!.getFirstChildElement("global")!.getFirstChildElement("dated")!.getFirstChildElement("sequencingMap"); // get the global sequencingMap (or null if there is none)
        const parts = this.getRootElement()!.getChildElements("part");                                 // get all the parts

        // expand global maps
        if (globalSequencingMap !== null) {
            const maps = this.getRootElement()!.getFirstChildElement("global")!.getFirstChildElement("dated")!.getChildElements();
            for (let j = 0; j < maps.size(); ++j) {                                                    // go through all maps
                ...
                if ((map.getChildCount() === 0)                                                         // do not expand sequencingMaps
                    ...
                    || map.getLocalName() === "sequencingMap")                                           // or if the map is empty
        ...
        for (let i = 0; i < parts.size(); ++i) {                                                        // for each part
     ...
     */
    static applySequencingMapToMap(sequencingMap: Element, map: Element, repetitionIDs: Map<string, string>): Element | null {
        ...
        for (let i = 0; i < gs.size(); ++i) {                          // fill the goto list, go through all gotos
            try {
                gotos.push(new Goto(gs.get(i)));                        // from the goto element create a Goto instance
            } catch (e) {                                               // if this fails
        ...
        for (let i = 0; i < gotos.length; ++i) {                       // find the next goto
            ...
            for (let e = Msm.getElementAtAfter(currentDate, map); e !== null; e = Helper.getNextSiblingElement(e) as Element | null) { // go through the map elements
                ...
                if (repetitionCounter !== null) {                                                                             // this is not the first time we process this element
                    ...
                    if (id !== null) {                                                                                        // if it has an xml:id
                        ...
                        for (let r = reps - 1; r > 0; --r)
                            prevId = repetitionIDs.get(prevId)!;
        ...
        for (let e = Msm.getElementAtAfter(currentDate, map); e !== null; e = Helper.getNextSiblingElement(e) as Element | null) {
            currentDate = parseFloat(e.getAttributeValue("date")!);                                                           // read its date
            ...
            const repetitionCounter = e.getAttribute("repetitionCounter");                                                      // get the counter
            if (repetitionCounter !== null) {                                                                                    // this is not the first time
                const reps = 1 + parseInt(e.getAttributeValue("repetitionCounter")!);                                            // increase repetition counter
                e.getAttribute("repetitionCounter")!.setValue(String(reps));                                                      // write it to the attribute
    ...
    private renderMidi(bpm: number, generateProgramChanges: boolean, exportExpressive: boolean): Midi | null {
        ...
        this.parseMarkerMap(this.getRootElement()!.getFirstChildElement("global")!, track, exportExpressive);
        this.parseTimeSignatureMap(this.getRootElement()!.getFirstChildElement("global")!, track, exportExpressive);
        this.parseKeySignatureMap(this.getRootElement()!.getFirstChildElement("global")!, track, exportExpressive);

        for (let part = this.getRootElement()!.getFirstChildElement("part"); part !== null; part = Helper.getNextSiblingElement("part", part)) {
            ...
            if (reallyGenerateProgramChanges) {
                reallyGenerateProgramChanges = !this.parseProgramChangeMap(part, track, chan, exportExpressive);
            }
    ...
    private makeInitialTempo(bpm: number, track: Track): void {
        ...
        try {
            beatlength = 1.0 / parseInt(this.getRootElement()!.getFirstChildElement("global")!.getFirstChildElement("dated")!.getFirstChildElement("timeSignatureMap")!.getFirstChildElement("timeSignature")!.getAttributeValue("denominator")!);
        } catch (e) {
            beatlength = 0.25;
        }
    ...
    private processPartName(part: Element, track: Track, channel: number, generateProgramChanges: boolean): void {
        if (part.getAttribute("name") === null || part.getAttributeValue("name") === '') {
            if (generateProgramChanges) {
                const event = EventMaker.createProgramChange(channel, 0, EventMaker.PC_Acoustic_Grand_Piano);
                if (event !== null) track.add(event);
            }
            return;
        }
    ...
    private parseProgramChangeMap(part: Element, track: Track, channel: number, exportExpressive: boolean): boolean {
        if (part.getFirstChildElement("dated") === null) return false;

        const programChangeMap = part.getFirstChildElement("dated")!.getFirstChildElement("programChangeMap");
        if (programChangeMap === null || programChangeMap.getChildCount() === 0) return false;

        let weHaveAnInitialPrgCh = false;
        for (let n = programChangeMap.getFirstChildElement("programChange"); n !== null; n = Helper.getNextSiblingElement("programChange", n)) {
            const date = exportExpressive ? Msm.readMillisecondsDateFromElement(n) : Math.round(parseFloat(Helper.getAttributeValue("date", n)));
    ...
    private processScore(part: Element, track: Track, exportExpressive: boolean): void {
        ...
        for (let n = part.getFirstChildElement("dated")!.getFirstChildElement("score")!.getFirstChildElement("note"); n !== null; n = Helper.getNextSiblingElement("note", n)) {
            ...
            if (exportExpressive) {
                ...
                const velocity = velocityAtt === null ? 100 : Math.round(parseFloat(velocityAtt.getValue()));
                ...
                if (endAtt === null) {
                    console.error("Missing attribute \"milliseconds.date.end\" in element " + n.toXML() + ". Using attribute \"duration\" instead.");
    ...
    private parseChannelVolumeMap(part: Element, track: Track, exportExpressive: boolean): void {
        ...
        const chan = parseInt(part.getAttributeValue("midi.channel")!);
        const cvMap = Helper.getFirstChildElement("channelVolumeMap", part.getFirstChildElement("dated")!);
        ...
        for (let i = es.size() - 1; i >= 0; --i) {
            ...
            if (!mandatory && (date >= (prevDate - Msm.CONTROL_CHANGE_DENSITY)))
                continue;
            prevDate = date;
    ...
    private parseKeySignatureMap(part: Element, track: Track, exportExpressive: boolean): void {
        ...
        for (let e = part.getFirstChildElement("dated")!.getFirstChildElement("keySignatureMap")!.getFirstChildElement("keySignature"); e !== null; e = Helper.getNextSiblingElement("keySignature", e)) {
            let date: number;
            if (exportExpressive) {
            ...
            let accids = 0;
            for (let a = e.getFirstChildElement("accidental"); a !== null; a = Helper.getNextSiblingElement("accidental", a)) {
                if (a.getAttribute("value") !== null) {
                    const value = parseFloat(a.getAttributeValue("value")!);
                    if (value > 1.0) { accids++; continue; }
                    if (value < 1.0) { accids--; }
                }
            }
    ...
    private parseTimeSignatureMap(part: Element, track: Track, exportExpressive: boolean): void {
        ...
        for (let e = part.getFirstChildElement("dated")!.getFirstChildElement("timeSignatureMap")!.getFirstChildElement("timeSignature"); e !== null; e = Helper.getNextSiblingElement("timeSignature", e)) {
            let date: number;
            if (exportExpressive) date = Msm.readMillisecondsDateFromElement(e);
            else date = Math.round(parseFloat(e.getAttributeValue("date")!));

            const numerator = e.getAttribute("numerator") === null ? 4 : Math.round(parseFloat(e.getAttributeValue("numerator")!));
            const denominator = e.getAttribute("denominator") === null ? 4 : Math.round(parseFloat(e.getAttributeValue("denominator")!));
    ...
    private parseMarkerMap(part: Element, track: Track, exportExpressive: boolean): void {
        ...
        for (let e = part.getFirstChildElement("dated")!.getFirstChildElement("markerMap")!.getFirstChildElement("marker"); e !== null; e = Helper.getNextSiblingElement("marker", e)) {
            let message: string;
            try {
            ...
            if (exportExpressive)
                ...
                event = EventMaker.createMarker(Math.round(parseFloat(e.getAttributeValue("date")!)), message);
            if (event !== null) track.add(event);
        }
    }
    ...
    private fitVelocities(min: number, max: number): void {
        ...
        if (!(scaleLowerHalf || scaleUpperHalf))
            return;

        // otherwise we need to apply compression
        console.log("Warning: velocity values [" + lowest + ", " + highest + "] break the specified limits [" + min + ", " + max + "] and will be compressed.");
     ...
     * @param max
     */
    private static computePartwiseCompression(attributes: KeyValue<number, Attribute>[], lowest: number, highest: number, min: number, max: number): void {
        ...
        let upperRolloff1 = 0.0, upperRolloff2 = 0.0, lowerRolloff1 = 0.0, lowerRolloff2 = 0.0;
        ...
        for (const attribute of attributes) {
            ...
            if (x < lowerCompMax) {
                result = (x >= min) ? (lowerRolloff2 * (x - min)) + lowerRaise : (lowerRolloff1 * (x - lowest)) + min;
            } else if (x > upperCompMin) {
                result = (x <= max) ? (upperRolloff1 * (x - upperCompMin)) + upperCompMin : (upperRolloff2 * (x - max)) + upperRaise;
            } else {
                continue;
            }
    ...
    getEndDate(): number {
        ...
        for (let i = 0; i < parts.size(); ++i) {                                                   // in each part
            ...
            for (let j = notes.size() - 1; j >= 0; --j) {                                          // go through all notes
    ...
    private static readMillisecondsDateFromElement(e: Element): number {
        ...
        if (dateAtt === null) {
            console.error("Missing attribute \"milliseconds.date\" in element " + e.toXML() + ". Using attribute \"date\" instead.");
    ...
    addIds(): number {
        ...
        const e: Nodes = root.query("descendant::*[(local-name()='note' or local-name()='rest') and not(@xml:id)]");
        for (let i = 0; i < e.size(); ++i)                                     // go through all the nodes
