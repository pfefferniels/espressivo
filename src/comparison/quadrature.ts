 ...
 *    equal endpoint signs, so an endpoint-sign test finds no bracket and the cell is integrated
 *    whole with the absolute value in the wrong place — measured relative error 1.48·10⁻², ten
 ...
 * cell dwarfs the running total — the shape of a window whose first cell is a `⊥` span priced
 * at `δ_row` and whose rest are small.
 */
...
/**
 * A running {@link neumaierSum}, same arithmetic and guarantees: the density layer walks a
 * refinement grid and would otherwise build an array per dimension per part just to add it up.
 */
 ...
 * across engines; `tests/comparison/quadrature.test.ts` re-derives them by Newton's method on
 * the Legendre polynomial `P₁₀` and asserts agreement. A derived table would instead be
 * self-consistent with its own bug. Only the five positive nodes are stored, the rule being
 * symmetric.
 */
...
/**
 * The same ten pairs, zipped once at module load, so the innermost loop of every distance in
 * this module indexes nothing. The zip is also an alignment check: two tables of different
 * length would silently produce a shorter rule.
 */
 ...
 *
 * Exact for polynomials of degree ≤ 19, hence exact on a step cell and near-machine on a smooth
 ...
 *
 * It is also only near-exact where `1/e − 1` is a non-negative integer (`e = 1, 1/2, 1/3, …`)
 * and degrades in between — 2.5·10⁻⁴ at `e = 0.76`. So this function is correct arithmetic for
 * the transformation it names and is called by no tempo evaluator here; the ruled scheme is
 * {@link integrateGradedPower}.
 */
 ...
 * negative, so `e = 0.23` would ask for zero panels and `e = 0.06` for a negative number. Two
 * is what the measurement behind the ruling ran — the quoted 3.3·10⁻⁶ worst case is this
 * function's.
 *
 * Growth is logarithmic because the boundary layer at `u = 1` has width `~1/e`: each extra
 * panel halves the layer's share of the last panel. `meanTempoAt = 0.999` (`e ≈ 693`) asks
 * for 12.
 */
 ...
 * late-weighted ritardando's boundary layer) and spread out for a small one. GL-10 runs on each
 * panel and the results are summed with compensation.
 *
 * `f` takes the raw curve argument `u`, not the powered one that
 ...
 *
 * A relative step rather than a fixed epsilon: these abscissae are ticks and can be 10⁵ or more,
 * where a fixed 1e−9 would be far below the representable spacing and round back to `high`.
 * `|high|·ε` is one ulp at that magnitude by construction. The fallback matters for correctness:
 * on an interval already about one ulp wide the step cannot land strictly inside, and returning
 * `high` would reinstate the closed probe this function exists to avoid.
 */
 ...
 * defect the absolute value has at its roots — so the crossings are resolved the same way: each
 * sign-constant piece is searched for one cap crossing by bisection and split there, leaving
 * pieces on which `min(|f|, cap)` is either `±f` or the constant `cap`, both integrated exactly.
 *
 * Completeness has the same caveat as {@link integrateAbsolute}: the caller's `splitPoints` must
 * isolate the crossings, one per sub-interval. On a piecewise-affine difference — what the
 * accentuation curve is between breakpoints — `|f|` is monotone on each sign-constant piece, so
 * one crossing per piece is a fact rather than an assumption.
 */
export function integrateCappedAbsolute(
  ...
  // Whether the cap bound on a piece is decided at the piece's MIDPOINT, which is sound because
  // the pieces below are split at every cap crossing: `|f| − cap` has one sign throughout each,
  // so any interior point decides it. Reporting this from inside the quadrature rather than
  // inferring it from the mass is the difference between "the cap bound somewhere" and "the cap
  // bound everywhere", which a mass comparison cannot tell apart.
 ...
 *
 * Shared rather than duplicated because the two must agree about where the roots are:
 * `integrateCappedAbsolute` with a cap larger than `max |f|` has to return exactly what
 * `integrateAbsolute` returns, and that identity is tested.
 */
 ...
 * elsewhere — and need not be roots themselves; they only have to separate the roots so each
 * sub-interval has at most one, which is what makes {@link bisectSignChange} complete.
 */
