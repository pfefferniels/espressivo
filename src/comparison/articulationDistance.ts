/**
 * The articulation distance.
 *
 * The compared object is not the atom but the per-anchor composed effective modifier: atoms
 * compose per note in map order (`articulationAtoms`), so what the renderer performs at an
 ...
 * offset, and the composition of two such maps is another one. The encoding-invariance
 ...
 *
 * The two families differ only in how the levers on one element combine, which
 ...
 * `−50`. The design prices the raw value as a document-level quantity for that reason — the
 ...
 * *which* note needs the MSM. So the design keeps the two kinds of anchor apart and the id-anchored
 * ones carry `datePositionKnown: false`.
 *
 * ## `d_articulation` has two components
 *
 ...
 * function as cells — which is the "absolutely continuous part plus atoms".
 */
...
export interface EffectiveModifier {
  ...
  /** Written onto the note and read by nothing — reported, never priced. */
...
export function anchorsOf(read: ArticulationAtoms): readonly ArticulationAnchor[] {
  ...
  //
  // Code-unit order, never `localeCompare` — `compare.ts` bans it by name for the report,
...
/** A replacement attribute: present-vs-absent is `⊥`, never a difference from a neutral. */
...
/**
 * `d(modifier_A, modifier_B)` — the `Σ_live-rows d_row` over the composed effects.
 *
 ...
 *
 * `⊥` for a replacement present on one side only is the whole reason for that paragraph:
 * a structural finding would contribute 0 and give `A=2, B=absent, C=100` the zero-set
 * violation `d(A,B) = d(B,C) = 0 < d(A,C)`.
 */
export function modifierDistance(
  ...
  // detuneCents / detuneHz are inert: read, written onto the note, consumed by nothing.
  // the rule is zero density, reported where the documents differ, so they are deliberately
  // absent from this sum.
  return total.total;
}

/** A difference in an inert attribute: reported, never priced. */
...
export interface ArticulationDistance {
   ...
   * rather than bit for bit. The table closes on this decomposition, which is why the
   ...
   * the window rather than placed. The report states it.
   */
  readonly datePositionKnown: boolean;
  /**
...
/**
 * `d_articulation` over the window — the alignment's own optimum.
 *
 * The alignment is the distance: the design makes the minimized functional the semantic definition,
 * so this function supplies the two costs and hands the argmin's value back. An anchor outside
 * the window is dropped before aligning, because the window is what every other dimension
...
export function articulationDistance(
  ...
  // An id-anchored atom has no known date, so the window cannot exclude it: it is
 ...
 * the `step` epsilon family, exactly, and it is why this component adds no numerical error
 * to the one the alignment already carries.
 *
 * A step with no def in force — the two cancelling dispositions, and the whole
...
/** the cap events, counted over the chosen alignment (see {@link ArticulationDistance}). */
