...
export class Author extends AbstractXmlSubtree {
    ...
    static createAuthor(xmlOrName: Element | string, number?: number | null, id?: string | null): Author | null {
        try {
    ...
    setName(name: string): void { this.nameText!.setValue(name); }
    getName(): string { return this.nameText!.getValue(); }

    setNumber(number: number | null): void {
        if (number === null) {
            if (this.number !== null) { this.number.detach(); this.number = null; }
            return;
        }
        if (this.number === null) {
