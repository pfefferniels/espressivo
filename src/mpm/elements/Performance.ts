...
export class Performance extends AbstractXmlSubtree {
    ...
    private pulsesPerQuarter: number = 720;
    ...
    static createPerformance(nameOrXml: string | Element, pulsesPerQuarter?: number, id?: string): Performance | null {
        try {
    ...
    protected parseData(xml: Element): void {
        ...
        if (name === null || name.getValue() === '') throw new Error("Cannot generate Performance object. Attribute name is missing or empty.");
        this.setXml(xml);
        ...
        if (ppqAtt === null) { ppqAtt = new Attribute("pulsesPerQuarter", "720"); this.getXml()!.addAttribute(ppqAtt); this.pulsesPerQuarter = 720; }
        else { this.pulsesPerQuarter = parseInt(ppqAtt.getValue()); }

        const globalElt = Helper.getFirstChildElement("global", this.getXml()!);
        if (globalElt === null) { this.global = Global.createGlobal()!; this.getXml()!.appendChild(this.global.getXml()!); }
        else { this.global = Global.createGlobal(globalElt); }
    ...
    size(): number { return this.parts.length; }
    getAllParts(): Part[] { return this.parts; }
    ...
    getPart(arg1: number | string, arg2?: number): Part | null {
        if (typeof arg1 === 'string') { for (const p of this.parts) if (p.getName() === arg1) return p; return null; }
        if (arg2 !== undefined) { for (const p of this.parts) if (p.getMidiChannel() === arg1 && p.getMidiPort() === arg2) return p; return null; }
        for (const p of this.parts) if (p.getNumber() === arg1) return p; return null;
    }

    addPart(part: Part): boolean {
        if (part === null || this.parts.includes(part)) return false;
        const parent = part.getXml()!.getParent();
        if (parent === null || parent !== this.getXml()) { part.getXml()!.detach(); this.getXml()!.appendChild(part.getXml()!); }
    ...
    removePartByNumber(number: number): void {
        for (let i = this.parts.length - 1; i >= 0; i--) {
            if (this.parts[i].getNumber() === number) { this.getXml()!.removeChild(this.parts[i].getXml()!); this.parts.splice(i, 1); }
        }
    }

    removePartByName(name: string): void {
        for (let i = this.parts.length - 1; i >= 0; i--) {
            if (this.parts[i].getName() === name) { this.getXml()!.removeChild(this.parts[i].getXml()!); this.parts.splice(i, 1); }
        }
    }

    removePart(part: Part): void {
        const idx = this.parts.indexOf(part);
        if (idx !== -1) { this.getXml()!.removeChild(part.getXml()!); this.parts.splice(idx, 1); }
    }
    ...
    getPulsesPerQuarter(): number { return this.pulsesPerQuarter; }
    getPPQ(): number { return this.getPulsesPerQuarter(); }
    setPulsesPerQuarter(ppq: number): void { this.pulsesPerQuarter = ppq; Helper.getAttribute("pulsesPerQuarter", this.getXml()!)!.setValue(String(ppq)); }
    setPPQ(ppq: number): void { this.setPulsesPerQuarter(ppq); }
    ...
    getCorrespondingPart(msmPart: Element | null): Part | null {
        ...
        if (mpmPart === null) { mpmPart = this.getPart(Helper.getAttributeValue("name", msmPart)); }
        return mpmPart;
    }

    private static addMsmMapToList(mapName: string, msmDated: Element | null, list: GenericMap[]): GenericMap | null {
    ...
    private static addPerformanceTimingAttributes(map: GenericMap | null): void {
        ...
        for (const e of map.getAllElements()) {
            ...
            if (duration !== null) e.getValue().addAttribute(new Attribute("duration.perf", duration.getValue()));
            const dateEnd = Helper.getAttribute("date.end", e.getValue());
            if (dateEnd !== null) e.getValue().addAttribute(new Attribute("date.end.perf", dateEnd.getValue()));
        }
    }
    ...
    perform(msm: Msm): Msm {
        ...
        // get global mpm maps
        const globalRubatoMap = this.getGlobal()!.getDated()!.getMap(Mpm.RUBATO_MAP) as RubatoMap | null;
        const globalTempoMap = this.getGlobal()!.getDated()!.getMap(Mpm.TEMPO_MAP) as TempoMap | null;
        const globalAsynchronyMap = this.getGlobal()!.getDated()!.getMap(Mpm.ASYNCHRONY_MAP) as AsynchronyMap | null;
        const globalImprecisionMap_timing = this.getGlobal()!.getDated()!.getMap(Mpm.IMPRECISION_MAP_TIMING) as ImprecisionMap | null;
        const globalImprecisionMap_dynamics = this.getGlobal()!.getDated()!.getMap(Mpm.IMPRECISION_MAP_DYNAMICS) as ImprecisionMap | null;
        const globalImprecisionMap_toneduration = this.getGlobal()!.getDated()!.getMap(Mpm.IMPRECISION_MAP_TONEDURATION) as ImprecisionMap | null;
        const globalImprecisionMap_tuning = this.getGlobal()!.getDated()!.getMap(Mpm.IMPRECISION_MAP_TUNING) as ImprecisionMap | null;
        const globalDynamicsMap = this.getGlobal()!.getDated()!.getMap(Mpm.DYNAMICS_MAP) as DynamicsMap | null;
        const globalMovementMap = this.getGlobal()!.getDated()!.getMap(Mpm.MOVEMENT_MAP) as MovementMap | null;
        const globalMetricalAccentuationMap = this.getGlobal()!.getDated()!.getMap(Mpm.METRICAL_ACCENTUATION_MAP) as MetricalAccentuationMap | null;
        const globalOrnamentationMap = this.getGlobal()!.getDated()!.getMap(Mpm.ORNAMENTATION_MAP) as OrnamentationMap | null;
        const globalArticulationMap = this.getGlobal()!.getDated()!.getMap(Mpm.ARTICULATION_MAP) as ArticulationMap | null;
        let maps: GenericMap[] = [];
        ...
        const globalTimeSignatureMap = Performance.addMsmMapToList("timeSignatureMap", globalDated, maps);
        ...
        if (globalImprecisionMap_timing !== null) globalImprecisionMap_timing.renderImprecisionToMap(globalPedalMap, true);
        ...
        for (let p = 0; p < parts.size(); ++p) {
            ...
            if (mpmPart === null)
                console.error("No MPM part found that corresponds to MSM part " + Helper.getAttributeValue("number", msmPart) + " \"" + Helper.getAttributeValue("name", msmPart) + "\"");
            else
                console.log("Performing part " + mpmPart.getNumber() + ": " + mpmPart.getName());
            ...
            if (mpmPart !== null) {
                ...
                metricalAccentuationMap = mpmPart.getDated()!.getMap(Mpm.METRICAL_ACCENTUATION_MAP) as MetricalAccentuationMap | null;
                ornamentationMap = mpmPart.getDated()!.getMap(Mpm.ORNAMENTATION_MAP) as OrnamentationMap | null;
                articulationMap = mpmPart.getDated()!.getMap(Mpm.ARTICULATION_MAP) as ArticulationMap | null;
                imprecisionMap_timing = mpmPart.getDated()!.getMap(Mpm.IMPRECISION_MAP_TIMING) as ImprecisionMap | null;
                imprecisionMap_dynamics = mpmPart.getDated()!.getMap(Mpm.IMPRECISION_MAP_DYNAMICS) as ImprecisionMap | null;
                imprecisionMap_toneduration = mpmPart.getDated()!.getMap(Mpm.IMPRECISION_MAP_TONEDURATION) as ImprecisionMap | null;
                imprecisionMap_tuning = mpmPart.getDated()!.getMap(Mpm.IMPRECISION_MAP_TUNING) as ImprecisionMap | null;
            }
            ...
            if (imprecisionMap_toneduration === null) imprecisionMap_toneduration = globalImprecisionMap_toneduration;
            ...
            if (metricalAccentuationMap !== null) metricalAccentuationMap.renderMetricalAccentuationToMap(score, (timeSignatureMap !== null ? timeSignatureMap : globalTimeSignatureMap), this.getPPQ());
            if (articulationMap !== null) articulationMap.renderArticulationToMap_noMillisecondModifiers(score);

            for (const m of maps)
                if (rubatoMap !== null) rubatoMap.renderRubatoToMap(m);

            if (ornamentationMap !== null) ornamentationMap.renderOrnamentationToMap(score);

            for (const m of maps)
                Performance.renderTempoToMap(m, this.getPPQ(), tempoMap);

            // pedalMap
            if (asynchronyMap !== null) asynchronyMap.renderAsynchronyToMap(pedalMap);
            if (imprecisionMap_timing !== null) imprecisionMap_timing.renderImprecisionToMap(pedalMap, true);
            ...
            if (articulationMap !== null) articulationMap.renderArticulationToMap_millisecondModifiers(score);
            ...
            if (imprecisionMap_dynamics !== null) imprecisionMap_dynamics.renderImprecisionToMap(score, true);
            if (imprecisionMap_toneduration !== null) imprecisionMap_toneduration.renderImprecisionToMap(score, true);
    ...
    private getAllMsmPartsAffectedByGlobalMap(msm: Msm, mapType: string): Element[] {
        ...
        for (let i = 0; i < parts.size(); ++i)
            msmPartsWithoutLocalMap.push(parts.get(i));

        for (const part of this.getAllParts()) {
            if (part.getDated()!.getMap(mapType) !== null) {
                const msmPart = msm.getPart(part.getNumber(), part.getName(), part.getMidiChannel(), part.getMidiPort());
                if (msmPart !== null) {
     ...
     */
    private static renderTempoToMap(map: GenericMap | null, ppq: number, tempoMap: TempoMap | null): void {
        if (tempoMap !== null) { tempoMap.renderTempoToMap(map, ppq); return; }
        if (map === null) return;
        for (let i = 0; i < map.size(); ++i) {
            ...
            if (endAtt !== null) e.addAttribute(new Attribute("milliseconds.date.end", endAtt.getValue()));
            else {
     ...
     */
    private static renderMillisecondsModifiersToMap(map: GenericMap | null, ornamentationMap: OrnamentationMap | null): void {
