...
export enum FrameDomain { Ticks = "ticks", Milliseconds = "milliseconds" }
export enum NoteOffShift { False = "false", True = "true", Monophonic = "monophonic" }

export class TemporalSpread {
    frameStart: number = 0.0;
    private frameLength: number = 0.0;
    ...
    constructor(xml?: Element) {
        ...
        if (domain !== null && domain.getValue() === "milliseconds") this.frameDomain = FrameDomain.Milliseconds;
        ...
        if (noteoffAtt !== null) {
            switch (noteoffAtt.getValue()) {
                case "true": this.noteOffShift = NoteOffShift.True; break;
                case "monophonic": this.noteOffShift = NoteOffShift.Monophonic; break;
            }
        }
    ...
    setFrameLength(length: number): void { this.frameLength = Math.max(0.0, length); }
    getFrameLength(): number { return this.frameLength; }
    ...
    apply(chordSequence: Element[][]): void {
        ...
        if (chordSequence.length > 1) {
            for (let i = 0; i < chordSequence.length - 1; ++i) {
                const dateOffset = (Math.pow(i / (chordSequence.length - 1), this.intensity) * this.frameLength) + this.frameStart;
                previous = this.setOrnamentDateAtts(dateOffset, chordSequence[i], previous);
            }
        }
        this.setOrnamentDateAtts(this.frameStart + this.frameLength, chordSequence[chordSequence.length - 1], previous);
    }

    private setOrnamentDateAtts(dateOffset: number, chord: Element[], previous: Element[] | null): Element[] | null {
        let dateAttName: string, durAttName: string;
        switch (this.frameDomain) {
            case FrameDomain.Ticks: dateAttName = "ornament.date.offset"; durAttName = "ornament.duration"; break;
            case FrameDomain.Milliseconds: dateAttName = "ornament.milliseconds.date.offset"; durAttName = "ornament.milliseconds.duration"; break;
            default: return null;
        }
        for (const note of chord) {
            const ornamentDateAtt = Helper.getAttribute(dateAttName, note);
            if (ornamentDateAtt !== null) ornamentDateAtt.setValue(String(dateOffset + parseFloat(ornamentDateAtt.getValue())));
            else note.addAttribute(new Attribute(dateAttName, String(dateOffset)));
        }
        switch (this.noteOffShift) {
            case NoteOffShift.False: return null;
            ...
            case NoteOffShift.Monophonic:
                if (previous !== null) {
                    for (const prev of previous) {
                        ...
                        if (ornamentDurationAtt !== null) ornamentDurationAtt.setValue(String(dateOffset - parseFloat(prevDateOffsetAtt.getValue())));
                        else prev.addAttribute(new Attribute(durAttName, String(dateOffset - parseFloat(prevDateOffsetAtt.getValue()))));
                    }
                }
                return chord;
            default: return null;
    ...
    setXml(xml: Element): void { this.xml = xml; }
    getXml(): Element { if (this.xml === null) return this.generateXML(); return this.xml; }

    generateXML(): Element {
        const ts = new Element("temporalSpread", Mpm.MPM_NAMESPACE);
        if (this.frameStart !== 0.0) ts.addAttribute(new Attribute("frame.start", String(this.frameStart)));
        if (this.frameLength !== 0.0) ts.addAttribute(new Attribute("frameLength", String(this.frameLength)));
        if (this.frameDomain === FrameDomain.Milliseconds) ts.addAttribute(new Attribute("time.unit", "milliseconds"));
        if (this.intensity !== 1.0) ts.addAttribute(new Attribute("intensity", String(this.intensity)));
        if (this.noteOffShift === NoteOffShift.True) ts.addAttribute(new Attribute("noteoff.shift", "true"));
        else if (this.noteOffShift === NoteOffShift.Monophonic) ts.addAttribute(new Attribute("noteoff.shift", "monophonic"));
        if (this.id !== null && this.id !== '') ts.addAttribute(new Attribute("xml:id", "http://www.w3.org/XML/1998/namespace", this.id));
        this.setXml(ts);
        return this.xml!;
    }

    toXml(): string { if (this.xml === null) return ""; return this.xml.toXML(); }

    setId(id: string | null): void {
        let idAtt = Helper.getAttribute("id", this.getXml());
        if (id === null) {
            if (idAtt !== null) { idAtt.detach(); this.id = null; }
            return;
        }
        if (idAtt === null) {
            this.id = id;
...
export class DynamicsGradient {
    transitionFrom: number = 0.0;
    transitionTo: number = 0.0;
    private id: string | null = null;
    private xml: Element | null = null;
    ...
    apply(chordSequence: Element[][], scale: number): void {
        if (chordSequence.length > 1) {
            const constFac = (scale * (this.transitionTo - this.transitionFrom)) / (chordSequence.length - 1);
    ...
    generateXML(): Element {
        ...
        if (this.transitionFrom !== 0.0) dg.addAttribute(new Attribute("transition.from", String(this.transitionFrom)));
        if (this.transitionTo !== this.transitionFrom) dg.addAttribute(new Attribute("transition.to", String(this.transitionTo)));
        if (this.id !== null && this.id !== '') dg.addAttribute(new Attribute("xml:id", "http://www.w3.org/XML/1998/namespace", this.id));
        this.setXml(dg);
        return this.xml!;
    }
...
export class OrnamentDef extends AbstractDef {
        ...
        if (children) {
            for (const transformer of children) {
                switch (transformer.getLocalName()) {
                    case "dynamicsGradient": this.dynamicsGradient = new DynamicsGradient(transformer); break;
                    case "temporalSpread": this.temporalSpread = new TemporalSpread(transformer); break;
                }
            }
        }
    }
    ...
    getTemporalSpread(): TemporalSpread | null { return this.temporalSpread; }
    setTemporalSpread(ts: TemporalSpread | null): void {
        ...
        while (old !== null) { this.getXml()!.removeChild(old); old = Helper.getFirstChildElement("temporalSpread", this.getXml()!); }
        if (ts !== null) this.getXml()!.appendChild(ts.generateXML());
    }

    setTemporalSpreadValues(frameStart: number, frameLength: number, frameDomain: FrameDomain, intensity: number, noteOffShift: NoteOffShift): void {
    ...
    getDynamicsGradient(): DynamicsGradient | null { return this.dynamicsGradient; }
    setDynamicsGradient(dg: DynamicsGradient | null): void {
        ...
        while (old !== null) { this.getXml()!.removeChild(old); old = Helper.getFirstChildElement("dynamicsGradient", this.getXml()!); }
