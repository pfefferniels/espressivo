...
export class RubatoDef extends AbstractDef {
    ...
    private lateStart: number = 0.0;
    private earlyEnd: number = 1.0;
    ...
    private parseDataInternal(xml: Element): void {
        ...
        if (intensityAttr === null) {
            ...
            intensityAttr.setValue(String(RubatoDef.ensureIntensityBoundaries(parseFloat(intensityAttr.getValue()))));
        }
        ...
        const le = RubatoDef.ensureLateStartEarlyEndBoundaries(parseFloat(lateStartAttr.getValue()), parseFloat(earlyEndAttr.getValue()));
    ...
    static createRubatoDef(name: string, frameLength: number, intensity: number, lateStart: number, earlyEnd: number): RubatoDef | null;
    static createRubatoDef(xml: Element): RubatoDef | null;
    static createRubatoDef(nameOrXml: string | Element, frameLength?: number, intensity?: number, lateStart?: number, earlyEnd?: number): RubatoDef | null {
        try {
    ...
    getLateStart(): number { return this.lateStart; }
    ...
    getEarlyEnd(): number { return this.earlyEnd; }
    ...
    private static ensureLateStartEarlyEndBoundaries(lateStart: number, earlyEnd: number): KeyValue<number, number> {
