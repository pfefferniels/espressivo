...
export class AccentuationPatternDef extends AbstractDef {
    ...
    static createAccentuationPatternDef(name: string, length: number, id: string): AccentuationPatternDef | null;
    static createAccentuationPatternDef(xml: Element): AccentuationPatternDef | null;
    static createAccentuationPatternDef(nameOrXml: string | Element, length?: number, id?: string): AccentuationPatternDef | null {
        try {
    ...
    addAccentuation(beat: number, value: number, transitionFrom: number, transitionTo: number, id?: string | null): number {
        ...
        const index = this.addAccentuationToArrayList([beat, value, transitionFrom, transitionTo], accElt);
    ...
    getAllAccentuations(): KeyValue<number[], Element>[] { return this.accentuations; }
    ...
    getAccentuationAt(beatPosition: number): number {
        ...
        for (let i = this.accentuations.length - 1; i >= 0; --i) {
            ...
            if (beatPosition > accentuation[0]) {
                if (i > (this.accentuations.length - 1))
                    segmentEnd = this.accentuations[i + 1].getKey()[0];
                break;
            }
        }
    ...
    size(): number { return this.accentuations.length; }
