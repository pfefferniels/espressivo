...
export class DistributionData {
    ...
    constructor(xml?: Element) {
        ...
        if (date !== null)
            this.startDate = parseFloat(date.getValue());
        ...
        if (seedAttr !== null)
            this.seed = parseInt(seedAttr.getValue());

        const lowerLimitAttr = xml.getAttribute("limit.lower");
        if (lowerLimitAttr !== null)
            this.lowerLimit = parseFloat(lowerLimitAttr.getValue());

        const upperLimitAttr = xml.getAttribute("limit.upper");
        if (upperLimitAttr !== null)
            this.upperLimit = parseFloat(upperLimitAttr.getValue());

        const lowerClipAttr = xml.getAttribute("clip.lower");
        if (lowerClipAttr !== null)
            this.lowerClip = parseFloat(lowerClipAttr.getValue());

        const upperClipAttr = xml.getAttribute("clip.upper");
        if (upperClipAttr !== null)
            this.upperClip = parseFloat(upperClipAttr.getValue());

        const modeAttr = xml.getAttribute("mode");
        if (modeAttr !== null)
            this.mode = parseFloat(modeAttr.getValue());
        ...
        if (maxStepWidthAttr !== null)
            this.maxStepWidth = parseFloat(maxStepWidthAttr.getValue());
        ...
        for (let i = 0; i < measurements.size(); i++) {
            ...
            if (value !== null)
                this.distributionList.push(parseFloat(value.getValue()));
        }
    }
