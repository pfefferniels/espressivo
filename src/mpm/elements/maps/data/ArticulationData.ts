...
export class ArticulationData {
    ...
    absoluteDurationChange: number = 0.0;
    absoluteDurationMs: number | null = null;
    absoluteDurationChangeMs: number = 0.0;
    relativeDuration: number = 1.0;
    absoluteDelay: number = 0.0;
    absoluteDelayMs: number = 0.0;
    absoluteVelocity: number | null = null;
    absoluteVelocityChange: number = 0.0;
    relativeVelocity: number = 1.0;
    detuneCents: number = 0.0;
    detuneHz: number = 0.0;
    ...
    constructor(xml?: Element) {
        ...
        if (absoluteDelayAttr !== null)
            this.absoluteDelay = parseFloat(absoluteDelayAttr.getValue());
        ...
        if (detuneCentsAttr !== null)
            this.detuneCents = parseFloat(detuneCentsAttr.getValue());

        const detuneHzAttr = xml.getAttribute("detuneHz");
        if (detuneHzAttr !== null)
            this.detuneHz = parseFloat(detuneHzAttr.getValue());
    ...
    articulateNote(note: Element | null): boolean {
        ...
        if (velocityAtt !== null) {
            ...
            if (this.absoluteVelocityChange !== 0.0) {
                velocityAtt.setValue(String(parseFloat(velocityAtt.getValue()) + this.absoluteVelocityChange));
                Helper.addToListAttribute(note, "modified", this.xmlId);
            }
        }
