...
export class AsynchronyMap extends GenericMap {
    ...
    static createAsynchronyMap(xml?: Element): AsynchronyMap | null {
        try { return xml !== undefined ? new AsynchronyMap(xml) : new AsynchronyMap("asynchronyMap"); } catch (e) { console.error(e); return null; }
    }
    ...
    getAsynchronyAt(date: number): number {
        ...
        if (i < 0) return 0.0;
        while (!this.elements[i].getValue().getLocalName().includes("asynchrony")) if (--i < 0) return 0.0;
    ...
    renderAsynchronyToMap(map: GenericMap | null): void {
        ...
        for (let asynIndex = 0; asynIndex < this.size(); ++asynIndex) {
            ...
            const asynEndDate = (asynIndex < (this.elements.length - 1)) ? this.elements[asynIndex + 1].getKey() : Number.MAX_VALUE;
            ...
            for (let mapIndex = 0; mapIndex < mapEntries.length; ++mapIndex) {
                ...
                if (mapEntry.getKey() >= this.elements[asynIndex].getKey()) {
                    ...
                    if (att !== null) { startDateMs = Math.max(0.0, parseFloat(att.getValue()) + offset); att.setValue(String(startDateMs)); Helper.addToListAttribute(mapEntry.getValue(), "modified", xmlId); }
                }
                const dur = Helper.getAttribute("duration", mapEntry.getValue());
                if (dur === null) { done.push(mapEntry); continue; }
                ...
                if (end >= this.elements[asynIndex].getKey()) {
                    ...
                    if (att !== null) { const ms = parseFloat(att.getValue()) + offset; att.setValue(String(Math.max(ms, startDateMs + 1))); Helper.addToListAttribute(mapEntry.getValue(), "modified", xmlId); }
                }
                done.push(mapEntry);
            }
            for (const removeMe of done) { const idx = mapEntries.indexOf(removeMe); if (idx !== -1) mapEntries.splice(idx, 1); }
            done.length = 0;
        }
    }
