...
export class ImprecisionMap extends GenericMap {
    ...
    protected parseData(xml: Element): void {
        ...
        if (!localname.includes("imprecisionMap"))
            throw new Error(`Cannot generate ImprecisionMap object. Local name "${xml.getLocalName()}" must contain "imprecisionMap".`);
    }

    setDomain(domain: string | null): void {
        if (domain === null || domain === '') { /* would set local name */ return; }
    ...
    addDistributionUniform(date: number, lowerLimit: number, upperLimit: number, seed?: number | null): number {
    ...
    addDistributionGaussian(date: number, standardDeviation: number, lowerLimit: number, upperLimit: number, seed?: number | null): number {
    ...
    addDistributionTriangular(date: number, lowerLimit: number, upperLimit: number, mode: number, lowerClip: number, upperClip: number, seed?: number | null): number {
    ...
    addDistributionBrownianNoise(date: number, maxStepWidth: number, lowerLimit: number, upperLimit: number, millisecondsTimingBasis: number, seed?: number | null): number {
    ...
    addDistributionCompensatingTriangle(date: number, degreeOfCorrelation: number, lowerLimit: number, upperLimit: number, lowerClip: number, upperClip: number, millisecondsTimingBasis: number, seed?: number | null): number {
        const e = new Element(DistributionData.COMPENSATING_TRIANGLE, Mpm.MPM_NAMESPACE);
        e.addAttribute(new Attribute("date", String(date)));
        e.addAttribute(new Attribute("degreeOfCorrelation", String(Math.max(degreeOfCorrelation, 0.0))));
        e.addAttribute(new Attribute("limit.lower", String(lowerLimit)));
        e.addAttribute(new Attribute("limit.upper", String(upperLimit)));
        e.addAttribute(new Attribute("clip.lower", String(lowerClip)));
        e.addAttribute(new Attribute("clip.upper", String(upperClip)));
    ...
    renderImprecisionToMap(map: GenericMap | null, shakePolyphonicPart: boolean): void {
        ...
        switch (this.getDomain()) {
            case "timing": domain = ImprecisionMap.TIMING; break;
            case "dynamics": domain = ImprecisionMap.DYNAMICS; break;
            case "toneduration": domain = ImprecisionMap.TONEDURATION; break;
            case "tuning": domain = ImprecisionMap.TUNING; break;
            default: return;
        }

        const pendingDurations: { endDate: number; msDateEnd: number; attribute: Attribute }[] = [];
        const offsets: Map<number, KeyValue<number, Attribute>[]> = new Map();
        let mapIndex = 0;
                    ...
                    case ImprecisionMap.TIMING: {
                        ...
                        if (msEndAtt !== null) {
                            ...
                            pendingDurations.push({
                                endDate: parseFloat(Helper.getAttributeValue("milliseconds.date.end", mapEntry.getValue())),
                                msDateEnd: msDateEnd,
                                attribute: msEndAtt
                            });
                        }
        ...
        if (shakePolyphonicPart) {
            if (domain === ImprecisionMap.TIMING)
                ImprecisionMap.shakeTimingOffsets(offsets);
            else
                ImprecisionMap.shakeOffsets(offsets);
        }

        ImprecisionMap.addOffsetsToAttributes(offsets, domain);
    }

    private static addToOffsetsMap(offsetsMap: Map<number, KeyValue<number, Attribute>[]>, millisecondsDate: number, offsetAndAttribute: KeyValue<number, Attribute>): void {
    ...
    private static getHandoverValue(randomPrev: RandomNumberProvider | null, ddPrev: DistributionData | null, ddNext: DistributionData): number | null {
    ...
    private static shakeTimingOffsets(offsets: Map<number, KeyValue<number, Attribute>[]>): void {
        for (const [, entries] of offsets) {
            ...
            const keepOffset = Math.floor(Math.random() * entries.length);
            const pitchOffsetTuplet: Map<number, number> = new Map();
    ...
    private static shake(offset: number): number {
        ...
        if (offset < 0.0)
            return RandomNumberProvider.createRandomNumberProvider_triangularDistribution(offset, of_, of_, offset, of_).getValue(0);
        return RandomNumberProvider.createRandomNumberProvider_triangularDistribution(of_, offset, offset, of_, offset).getValue(0);
    }

    private static addOffsetsToAttributes(offsets: Map<number, KeyValue<number, Attribute>[]>, domain: number): void {
        for (const [, entries] of offsets) {
            for (const entry of entries) {
                ...
                if (domain === ImprecisionMap.TIMING)
                    ...
                    entry.getValue().setValue(String(attValue + entry.getKey()));
            }
        }
    }

    static renderImprecisionToMap(map: GenericMap | null, imprecisionMap: ImprecisionMap | null, shakePolyphonicPart: boolean): void {
...
GenericMap.registerMapFactory('imprecisionMap.timing', (xml) => ImprecisionMap.createImprecisionMap(xml));
GenericMap.registerMapFactory('imprecisionMap.dynamics', (xml) => ImprecisionMap.createImprecisionMap(xml));
GenericMap.registerMapFactory('imprecisionMap.toneduration', (xml) => ImprecisionMap.createImprecisionMap(xml));
GenericMap.registerMapFactory('imprecisionMap.tuning', (xml) => ImprecisionMap.createImprecisionMap(xml));
