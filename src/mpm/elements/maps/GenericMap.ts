 ...
 * import *it*, so a table here would be a nine-way cycle.
 *
...
export class GenericMap extends AbstractXmlSubtree {
  ...
  /**
   * Build a map from an element that {@link sourceElement} has already accepted. Cannot fail:
   * the name checks are that function's, and it answers with a `Result`.
   *
   * No virtual call — {@link indexElements} is `private`, so a subclass cannot be reached
   * before its own field initialisers have run. `ImprecisionMap`'s extra name check therefore
   * runs in its factory, after construction; see there for why the order matters.
   */
  ...
  /**
   * The element a map will be built from, or why the name it was given is not a map's. A map
   * name contains `"Map"` or is exactly `"score"`; the question is asked once, here, before
   * anything is built.
   *
  ...
  /**
   * The parsing half of every map factory: {@link sourceElement}'s checks, then the
   * subclass's constructor.
   *
   * `build` is a closure rather than a `new (…) => M` field because every map constructor is
   * `private`, which is what such a field may not hold — the same reason `maps/map.ts` spells
   * its `is` predicates out per row.
   *
   * `what` is the subclass's name and reaches only the {@link attemptParse} residue; the two
   * name checks report as `GenericMap`, because it is `GenericMap`'s rule they enforce.
   */
   ...
   * `names.ts` constant, which `maps/map.ts`'s table establishes is a map name — so there is
   * nothing left to refuse and nothing for the caller to check. `indexElements` on a childless
   * element cannot fail either: it reads no children and reorders none.
   */
   ...
   * class meets the "constructed by parsing an element" invariant in its constructor.
   *
   * Re-parsing a *different* element into a live map is not a supported operation — `Dated`
   * indexes maps by the type of the element they were built from — so this throws rather than
   * silently doing nothing. `styles/style.ts` states the same thing the same way.
   */
  ...
  /**
   * Create a map either from a local name (a fresh, empty map) or from an existing element
   * (parsed); `null` is accepted and reported on.
   *
   * Reports the reason instead of printing it: the MPM parse is best-effort and a malformed
   * map must not abort the surrounding document, but "which map, and why" stays available to
   * that document's reader.
   */
   ...
   *
   * The insertion is a backwards linear scan ({@link insertionIndexFor}) rather than an
   * append, so children already out of order in the source document are indexed in date
   * order. It finds the *last* position whose date is `<=` the new one, which makes the pass
  ...
  /**
   * Rewrite the XML children into the order {@link elements} is already in. Children this map
   * does not know about keep their relative order and so drift to the end, as
   ...
   * back in line. A stable insertion sort.
   *
   * The keys are the SYMBOLIC `@date`, as {@link indexElements} builds them and as every
   * lookup reads them ({@link getElementIndexBeforeAt}, {@link getAllElementsAt}); keying the
   * index on `date.perf` would break every symbolic lookup in the renderer.
   *
   * PARITY — Java swaps the two positions instead of shifting the elements between them
   * (`GenericMap.java`'s `Collections.swap(this.elements, i, moveToIndex)`), which strands
   * everything in between and leaves the array unsorted for any arrangement with more than
   * one displaced element. Repaired in the fork first (`meico@a1bdf254`) and here to match;
   * recorded in PARITY.md §3.
   *
   * No output moved, and nothing reachable today could move: the one caller is
   * `ArticulationMap`'s `if (mapTimingChanged) map.sort()` (`ArticulationMap.java:479` is
   * likewise the only `sort()` call in Java's whole `mpm` package), and articulation writes
   * `@date.perf`, `@duration.perf` and `@velocity` but never `@date` — so the keys are
   * unchanged and the pass finds nothing to move. The defect is reachable only by a caller
   * that edits `@date` on elements already in the map and then sorts.
   */
  sort(): void {
  ...
  /**
   * PARITY NOTE — a stub. Java's `GenericMap.setType` calls `Element.setLocalName()`, which
   * the XomTypes layer does not implement (xmldom nodes cannot be renamed in place), so the
   * validation still runs and still logs but the rename does not happen. A map's type is
   * therefore fixed at construction; nothing in the MEI/MSM ⇒ MIDI pipeline calls this.
   */
   ...
   * A lookup at a date the map does not carry answers with the next entry rather than with
   * nothing, as Java does (`GenericMap.getAllElementsAt`).
   *
   * The run of matching entries on a date-sorted index ends at the upper bound for `date`.
   * When the first entry is dated after `date` that bound sits at or before `start`, and the
   * `max` is what yields the single following entry.
   */
   ...
   *
   * They are not interchangeable: picking "the last element at or before this date" versus
   * "strictly before it" decides whether an instruction dated exactly on a note applies to
   * that note, and different callers need different answers. Each returns -1 for "no such
   * element".
   *
  ...
  protected insertElement(element: KeyValue<number, Element>, firstAtDate = false): number {
    ...
    // the last instruction — `addStyleSwitch` is the one caller — takes effect from the top of
    // the map.
   ...
   * 0 when there is none. Shared by {@link indexElements} and {@link insertElement}.
   *
   ...
   * this one decides what the order will be, and its date comes straight out of `parseFloat`,
   * which answers NaN for a malformed `@date`. Every comparison against NaN is false, so the
   * scan puts such an entry at 0 and walks past it afterwards, where a binary search would
   * probe it as a partition point and split on an answer false on both sides. The ordering is
   ...
   *
   * An index past the end is a no-op; a NEGATIVE one throws, naming the index and the bound.
   * There is no `< 0` guard because adding one would turn that throw into a silent no-op,
   ...
   *
   * Deliberately asymmetric: an index past the end reads the LAST entry, while a negative
   * index (or an empty map) means "nothing".
   *
   ...
   * attributes off and the key that is the instruction's start date. The bound is proved
   * here, once, for all eight `get*DataOf` accessors.
   *
   * The resolvers return an index rather than the entry because the accessors need it for
   * {@link nextDateOfType} and {@link findStyleNameAt} as well, and returning a pair would
   ...
   * `localName`, or `Number.MAX_VALUE` when there is none. This is Java's five per-class
   * `getEndDate(index)` methods, which differ only in the name they scan for — `tempo`,
   * `dynamics`, `rubato`, `accentuationPattern`, `movement`.
   *
   * `MAX_VALUE` and not null because `endDate` is the open end of a span the callers compare
   * dates against, and "there is no next one" means the span runs to the end of time.
   *
   ...
   * positional one). Answers with the highest-positioned switch dated at or before `date`.
   */
   ...
   * return type be one arm of the style union instead of the whole of it, so the caller needs
   * no `as TempoStyle | null` cast. `generic` resolves nothing, because it names no
  ...
  getStyle<K extends StyleKind>(kind: K, styleName: string | null): StyleOfKind<K> | null {
    ...
    // The global lookup stays lazy: it is a pure map read, so evaluating it eagerly would be
    // observationally identical, but this runs once per styled instruction and `980ae7e`
    // bought that path its linearity.
