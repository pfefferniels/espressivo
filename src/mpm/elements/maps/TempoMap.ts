...
export class TempoMap extends GenericMap {
    ...
    static createTempoMap(xml?: Element): TempoMap | null {
        try { return xml !== undefined ? new TempoMap(xml) : new TempoMap("tempoMap"); } catch (e) { console.error(e); return null; }
    }

    protected parseData(xml: Element): void { super.parseData(xml); }

    addTempo(date: number, bpm: string, beatLength: number): number;
    addTempo(date: number, bpm: string, transitionTo: string, beatLength: number, meanTempoAt: number): number;
    addTempo(date: number, bpm: string, transitionTo: string, beatLength: number, meanTempoAt: number, id: string): number;
    addTempo(data: TempoData): number;
    addTempo(dateOrData: number | TempoData, bpm?: string, transitionToOrBeatLength?: string | number, beatLength?: number, meanTempoAt?: number, id?: string): number {
        if (typeof dateOrData !== 'number') {
            ...
            else { console.error("Cannot add tempo, bpm not specified."); return -1; }
            if (data.transitionToString !== null) e.addAttribute(new Attribute("transition.to", data.transitionToString));
            else if (data.transitionTo !== null) e.addAttribute(new Attribute("transition.to", String(data.transitionTo)));
            if (data.meanTempoAt !== null) e.addAttribute(new Attribute("meanTempoAt", String(data.meanTempoAt)));
        ...
        if (typeof transitionToOrBeatLength === 'string') {
            ...
            if (id !== undefined) e.addAttribute(new Attribute("xml:id", "http://www.w3.org/XML/1998/namespace", id));
        } else {
    ...
    getTempoDataOf(index: number): TempoData | null {
        ...
        if (e.getLocalName() === "tempo") {
            ...
            for (let j = index; j >= 0; --j) {
                ...
                if (s.getLocalName() === "style") { td.styleName = Helper.getAttributeValue("name.ref", s); break; }
            }
            ...
            if (ttAtt !== null) {
                ...
                if (td.transitionTo === td.bpm) { td.transitionToString = null; td.transitionTo = null; }
                else {
                    const mtaAtt = Helper.getAttribute("meanTempoAt", e);
                    if (mtaAtt !== null) {
                        td.meanTempoAt = parseFloat(mtaAtt.getValue());
                        if (td.meanTempoAt <= 0.0) { td.bpmString = td.transitionToString; td.bpm = td.transitionTo; td.transitionToString = null; td.transitionTo = null; }
                        else if (td.meanTempoAt >= 1.0) { td.transitionToString = null; td.transitionTo = null; }
                        else { td.exponent = TempoMap.computeExponent(td.meanTempoAt); }
                    } else { td.meanTempoAt = 0.5; td.exponent = 1.0; }
                }
            }
            return td;
        }
    ...
    private getEndDate(index: number): number {
        ...
        for (let j = index + 1; j < this.elements.length; ++j) {
            if (this.elements[j].getValue().getLocalName() === "tempo") { endDate = this.elements[j].getKey(); break; }
        }
        return endDate;
    }

    private static computeExponent(meanTempoAt: number): number { return Math.log(0.5) / Math.log(meanTempoAt); }
    ...
    private static getTempoAtStatic(date: number, tempoData: TempoData | null): number {
        ...
        if (tempoData.exponent === null) tempoData.exponent = (tempoData.meanTempoAt === null) ? 1.0 : TempoMap.computeExponent(tempoData.meanTempoAt);
    ...
    renderTempoToMap(map: GenericMap | null, ppq: number): void {
        ...
        if (this.elements.length === 0) {
            for (; mapIndex < map.size(); ++mapIndex) {
                ...
                mapEntry.getValue().addAttribute(new Attribute("milliseconds.date.end", String(TempoMap.computeMillisecondsForNoTempo(endDate, ppq))));
            }
            return;
        }
        ...
        for (let tempoIndex = 0; tempoIndex < this.size(); ++tempoIndex) {
            ...
            for (let i = 0; i < pendingDurations.length; ++i) {
                ...
                map.elements[pd.getValue()].getValue().addAttribute(new Attribute("milliseconds.date.end", String(milliseconds)));
                pendingDurations.splice(i, 1);
                --i;
            }

            if ((mapIndex >= map.size()) && pendingDurations.length === 0)
                break;
        }
    }
    ...
    static computeDiffTiming(date: number, ppq: number, tempoData: TempoData | null): number {
        ...
        if (tempoData.isConstantTempo()) return TempoMap.computeMillisecondsForConstantTempo(date, ppq, tempoData);
        return TempoMap.computeMillisecondsForTempoTransition(date, ppq, tempoData);
    }

    private static computeMillisecondsForNoTempo(date: number, ppq: number): number { return (600.0 * date) / ppq; }
    private static computeMillisecondsForConstantTempo(date: number, ppq: number, tempoData: TempoData): number {
        return ((15000.0 * (date - tempoData.startDate)) / (tempoData.bpm! * tempoData.beatLength * ppq));
    }
    private static computeMillisecondsForTempoTransition(date: number, ppq: number, tempoData: TempoData): number {
        ...
        for (let k = 1; k < n; ++k) resultSum += 2.0 / TempoMap.getTempoAtStatic(tempoData.startDate + 2 * k * x, tempoData);
        for (let k = 1; k <= n; ++k) resultSum += 4.0 / TempoMap.getTempoAtStatic(tempoData.startDate + (2 * k - 1) * x, tempoData);
