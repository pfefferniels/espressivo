...
export class DynamicsMap extends GenericMap {
    ...
    static createDynamicsMap(xml?: Element): DynamicsMap | null {
        try { return xml !== undefined ? new DynamicsMap(xml) : new DynamicsMap("dynamicsMap"); } catch (e) { console.error(e); return null; }
    }

    protected parseData(xml: Element): void { super.parseData(xml); }

    addDynamics(date: number, volume: string, transitionTo?: string, curvature?: number, protraction?: number, subNoteDynamics?: boolean, id?: string): number {
        ...
        if (protraction !== undefined) e.addAttribute(new Attribute("protraction", String(protraction)));
    ...
    addDynamicsFromData(data: DynamicsData): number {
        ...
        else { console.error("Cannot add dynamics, volume not specified."); return -1; }
    ...
    getDynamicsDataAt(date: number): DynamicsData | null {
        for (let i = this.getElementIndexBeforeAt(date); i >= 0; --i) { const dd = this.getDynamicsDataOf(i); if (dd !== null) return dd; }
        return null;
    }
    ...
    getDynamicsDataOf(index: number): DynamicsData | null {
        ...
        dd.xml = e;
        const att = Helper.getAttribute("id", e); if (att !== null) dd.xmlId = att.getValue();
        for (let j = index; j >= 0; --j) { const s = this.elements[j].getValue(); if (s.getLocalName() === "style") { dd.styleName = Helper.getAttributeValue("name.ref", s); break; } }
        dd.style = this.getStyle(Mpm.DYNAMICS_STYLE, dd.styleName) as DynamicsStyle | null;
        const volAtt = Helper.getAttribute("volume", e); if (volAtt === null) return null;
        ...
        if (ttAtt !== null) { dd.transitionToString = ttAtt.getValue(); dd.transitionTo = DynamicsStyle.getNumericValueStatic(dd.transitionToString, dd.style); }
        else { dd.transitionToString = dd.volumeString; dd.transitionTo = dd.volume; dd.curvature = 0.0; dd.protraction = 0.0; }
        const sndAtt = Helper.getAttribute("subNoteDynamics", e); if (sndAtt !== null) dd.subNoteDynamics = sndAtt.getValue() === "true";
        return dd;
    }

    private getEndDate(index: number): number {
        for (let j = index + 1; j < this.elements.length; ++j) { if (this.elements[j].getValue().getLocalName() === "dynamics") return this.elements[j].getKey(); }
    ...
    renderDynamicsToMap(map: GenericMap | null): GenericMap | null {
        ...
        for (let dynamicsIndex = 0; dynamicsIndex < this.size(); ++dynamicsIndex) {
            ...
            if (chanVolMap !== null) {
                if (dd.subNoteDynamics && (dynamicsIndex < (this.size() - 1))) {
                    ...
                    for (; mapIndex < map.size(); ++mapIndex) {
                        ...
                        if (mapEntry.getKey() < dd.startDate || mapEntry.getValue().getLocalName() !== "note") continue;
                ...
                if (chanVolMap.isEmpty() || Helper.getAttributeValue("value", chanVolMap.getLastElement()!) !== "100.0") {
            ...
            for (; mapIndex < map.size(); ++mapIndex) {
                ...
                if (mapEntry.getKey() < dd.startDate) { mapEntry.getValue().addAttribute(new Attribute("velocity", "100.0")); continue; }
                if (mapEntry.getKey() >= dd.endDate!) break;
                mapEntry.getValue().addAttribute(new Attribute("velocity", String(dd.getDynamicsAt(mapEntry.getKey()))));
            }
        }
        return chanVolMap;
    }

    private static generateSubNoteDynamics(dynamicsData: DynamicsData, channelVolumeMap: GenericMap): void {
    ...
    static renderDynamicsToMap(map: GenericMap | null, dynamicsMap: DynamicsMap | null): GenericMap | null {
        if (dynamicsMap !== null) return dynamicsMap.renderDynamicsToMap(map);
        if (map === null) return null;
        for (let i = 0; i < map.size(); ++i) { const e = map.getElement(i)!; if (e.getLocalName() === "note") e.addAttribute(new Attribute("velocity", "100.0")); }
        return null;
    }
}
