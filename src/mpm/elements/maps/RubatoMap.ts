...
export class RubatoMap extends GenericMap {
    ...
    static createRubatoMap(xml?: Element): RubatoMap | null {
        try { return xml !== undefined ? new RubatoMap(xml) : new RubatoMap("rubatoMap"); } catch (e) { console.error(e); return null; }
    }

    protected parseData(xml: Element): void { super.parseData(xml); }

    addRubato(date: number, frameLength: number, intensity: number, lateStart: number, earlyEnd: number, loop: boolean): number;
    addRubato(date: number, rubatoDefName: string, loop: boolean): number;
    addRubato(data: RubatoData): number;
    addRubato(dateOrData: number | RubatoData, arg2?: number | string, arg3?: number | boolean, arg4?: number, arg5?: number, arg6?: boolean): number {
        if (typeof dateOrData !== 'number') {
            ...
            if (data.rubatoDefString !== null) e.addAttribute(new Attribute("name.ref", data.rubatoDefString));
            if (data.frameLength !== null) e.addAttribute(new Attribute("frameLength", String(data.frameLength)));
        ...
        if (typeof arg2 === 'string') { e.addAttribute(new Attribute("name.ref", arg2)); e.addAttribute(new Attribute("loop", String(arg3))); }
        else { e.addAttribute(new Attribute("frameLength", String(arg2))); e.addAttribute(new Attribute("intensity", String(arg3))); e.addAttribute(new Attribute("lateStart", String(arg4))); e.addAttribute(new Attribute("earlyEnd", String(arg5))); e.addAttribute(new Attribute("loop", String(arg6))); }
    ...
    getRubatoDataOf(index: number): RubatoData | null {
        ...
        rd.xml = e;
        const att = Helper.getAttribute("id", e); if (att !== null) rd.xmlId = att.getValue();
        for (let j = index; j >= 0; --j) { const s = this.elements[j].getValue(); if (s.getLocalName() === "style") { rd.styleName = Helper.getAttributeValue("name.ref", s); break; } }
        rd.style = this.getStyle(Mpm.RUBATO_STYLE, rd.styleName) as RubatoStyle | null;
        if (rd.style !== null) { const nrAtt = Helper.getAttribute("name.ref", e); if (nrAtt !== null) { rd.rubatoDefString = nrAtt.getValue(); rd.rubatoDef = rd.style.getDef(rd.rubatoDefString) ?? null; } }
        ...
        const loopAtt = Helper.getAttribute("loop", e); if (loopAtt !== null) rd.loop = loopAtt.getValue() === "true";
        const intAtt = Helper.getAttribute("intensity", e); if (intAtt !== null) rd.intensity = parseFloat(intAtt.getValue()); else if (rd.rubatoDef !== null) rd.intensity = rd.rubatoDef.getIntensity();
        const lsAtt = Helper.getAttribute("lateStart", e); if (lsAtt !== null) rd.lateStart = parseFloat(lsAtt.getValue()); else if (rd.rubatoDef !== null) rd.lateStart = rd.rubatoDef.getLateStart();
        const eeAtt = Helper.getAttribute("earlyEnd", e); if (eeAtt !== null) rd.earlyEnd = parseFloat(eeAtt.getValue()); else if (rd.rubatoDef !== null) rd.earlyEnd = rd.rubatoDef.getEarlyEnd();
        // ensure boundaries
        if (rd.lateStart !== null && rd.lateStart < 0.0) rd.lateStart = 0.0;
        if (rd.earlyEnd !== null && rd.earlyEnd > 1.0) rd.earlyEnd = 1.0;
        if (rd.lateStart !== null && rd.earlyEnd !== null && rd.lateStart >= rd.earlyEnd) { rd.lateStart = 0.0; rd.earlyEnd = 1.0; }
        return rd;
    }

    private getEndDate(index: number): number {
        for (let j = index + 1; j < this.elements.length; ++j) { if (this.elements[j].getValue().getLocalName() === "rubato") return this.elements[j].getKey(); }
    ...
    renderRubatoToMap(map: GenericMap | null): void {
        ...
        for (let rubIndex = 0; rubIndex < this.size(); ++rubIndex) {
            ...
            for (; mapIndex < map.size(); ++mapIndex) {
                ...
                if (mapEntry.getKey() >= rd.endDate! || (!rd.loop && mapEntry.getKey() >= (rd.startDate + rd.frameLength!))) break;

                const dateAtt = Helper.getAttribute("date.perf", mapEntry.getValue());
                if (dateAtt !== null)
                    dateAtt.setValue(String(RubatoMap.computeRubatoTransformation(parseFloat(dateAtt.getValue()), rd)));
