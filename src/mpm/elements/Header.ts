...
export class Header extends AbstractXmlSubtree {
    private readonly styleDefs: Map<string, Map<string, GenericStyle>> = new Map();
    ...
    addStyleType(typeOrXml: string | Element): Map<string, GenericStyle> | null {
        ...
        if (styleDefElements) {
            for (const styleDef of styleDefElements) {
                ...
                switch (type) {
                    case Mpm.ARTICULATION_STYLE: sd = ArticulationStyle.createArticulationStyle(styleDef); break;
                    case Mpm.TEMPO_STYLE: sd = TempoStyle.createTempoStyle(styleDef); break;
                    case Mpm.DYNAMICS_STYLE: sd = DynamicsStyle.createDynamicsStyle(styleDef); break;
                    case Mpm.METRICAL_ACCENTUATION_STYLE: sd = MetricalAccentuationStyle.createMetricalAccentuationStyle(styleDef); break;
                    case Mpm.RUBATO_STYLE: sd = RubatoStyle.createRubatoStyle(styleDef); break;
                    case Mpm.ORNAMENTATION_STYLE: sd = OrnamentationStyle.createOrnamentationStyle(styleDef); break;
                    default: sd = GenericStyle.createGenericStyle(styleDef);
                }
                if (sd === null) continue;
    ...
    getAllStyleTypes(): Map<string, Map<string, GenericStyle>> { return this.styleDefs; }
    getAllStyleDefs(type: string): Map<string, GenericStyle> | undefined { return this.styleDefs.get(type); }
    ...
    addStyleDef(type: string, styleDefOrName: GenericStyle | string): GenericStyle | null | void {
        if (typeof styleDefOrName === 'string') {
            ...
            switch (type) {
                case Mpm.DYNAMICS_STYLE: styleDef = DynamicsStyle.createDynamicsStyle(styleDefOrName); break;
                case Mpm.ARTICULATION_STYLE: styleDef = ArticulationStyle.createArticulationStyle(styleDefOrName); break;
                case Mpm.METRICAL_ACCENTUATION_STYLE: styleDef = MetricalAccentuationStyle.createMetricalAccentuationStyle(styleDefOrName); break;
                case Mpm.TEMPO_STYLE: styleDef = TempoStyle.createTempoStyle(styleDefOrName); break;
                case Mpm.RUBATO_STYLE: styleDef = RubatoStyle.createRubatoStyle(styleDefOrName); break;
                case Mpm.ORNAMENTATION_STYLE: styleDef = OrnamentationStyle.createOrnamentationStyle(styleDefOrName); break;
                default: styleDef = GenericStyle.createGenericStyle(styleDefOrName);
            }
    ...
    renameStyleDef(type: string, currentName: string, newName: string): GenericStyle | null {
        ...
        if (!allStyleDefs || allStyleDefs.size === 0) { console.log("There are no styleDef elements for type " + type); return null; }
        const styleDef = allStyleDefs.get(currentName);
        if (styleDef === undefined) { console.log(`There is no styleDef element with name "${currentName}" to be renamed.`); return null; }
    ...
    clear(): void { this.getXml()!.removeChildren(); this.styleDefs.clear(); }
}
