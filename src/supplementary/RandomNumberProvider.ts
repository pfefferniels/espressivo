...
export class RandomNumberProvider {
    ...
    private lowCut: number = 0;
    private highCut: number = 0;
    private standardDeviation: number = 0;

    private lowerLimit: number = 0;
    private upperLimit: number = 0;
    private maxStepWidth: number = 0;
    private mode: number = 0;
    private degreeOfCorrelation: number = 0;
    ...
    static createRandomNumberProvider_uniformDistribution(lowerLimit: number, upperLimit: number): RandomNumberProvider {
    ...
    static createRandomNumberProvider_gaussianDistribution(standardDeviation: number, lowerLimit: number, upperLimit: number): RandomNumberProvider {
    ...
    static createRandomNumberProvider_triangularDistribution(lowerLimit: number, upperLimit: number, mode: number, lowCut: number, highCut: number): RandomNumberProvider {
    ...
    static createRandomNumberProvider_brownianNoiseDistribution(maxStepWidth: number, lowerLimit: number, upperLimit: number): RandomNumberProvider {
        const rand = new RandomNumberProvider(RandomNumberProvider.DISTRIBUTION_CORRELATED_BROWNIANNOISE);
    ...
    static createRandomNumberProvider_compensatingTriangleDistribution(degreeOfCorrelation: number, lowerLimit: number, upperLimit: number, lowCut: number, highCut: number): RandomNumberProvider {
        const rand = new RandomNumberProvider(RandomNumberProvider.DISTRIBUTION_CORRELATED_COMPENSATING_TRIANGLE);
    ...
    getValue(index: number): number {
        ...
        while (this.series.length <= index)
            this.nextDouble();
    ...
    private compensatingTriangleDistribution(): number {
        ...
        const newLowerLimit = prevRandomNum - ((prevRandomNum - this.lowerLimit) / this.degreeOfCorrelation);
        const newUpperLimit = prevRandomNum + ((this.upperLimit - prevRandomNum) / this.degreeOfCorrelation);
    ...
    private brownianNoiseDistribution(): number {
        ...
        let attempts = 0;
        do {
            result = this.series[this.series.length - 1] + ((this.nextRandom() - 0.5) * 2.0 * this.maxStepWidth);
            if (++attempts > 10000) {
