/**
 * Tick ⇄ millisecond conversion under a `<tempo>` instruction, at mpmify's resolution.
 *
 * **The arithmetic is espressivo's, not mpmify's**, and it does not live here any more: it is
 * `src/mpm/timing.ts`, where every function that crosses between ticks and milliseconds takes
 * the document's `@pulsesPerQuarter` as an argument, the way `TempoMap.computeDiffTiming` does.
 * What remains here is that algebra bound to {@link PULSES_PER_QUARTER} — mpmify writes 720 and
 * only 720, so every call site would otherwise pass the same number — plus the two curve-fitting
 * helpers the desks drive, which are mpmify's own.
 *
 * The record of *why* the delegation exists is in the header of `src/mpm/timing.ts`: this module
 * used to carry hand-copies of `resolveTempo`, `tempoAt` and `computeDiffTiming`, Simpson's rule
 * included down to the sub-interval count, and they had drifted in four ways — `meanTempoAt` of
 * exactly 1 reading as ±Infinity, one above 1 overshooting both endpoints, a negative one giving
 * `NaN`, and a `|| 0.5` that turned an explicit `meanTempoAt="0"` into a linear ramp. Do not
 * reintroduce a local copy of any of them.
 *
 * One consequence is mpmify's alone and stated here rather than there: a document with a
 * malformed `@meanTempoAt` now measures as `NaN` throughout its span, which `auditInstructions`
 * refuses to let a transformer leave standing. That is the intended outcome — it is what the
 * renderer produces, and the renderer's answer is the one that decides whether a fit is right.
 */
import { tempoAt, type Tempo as ResolvedTempo } from '../../../mpm/elements/maps/data/tempo.js';
import {
...
import type { InstructionOptions } from '../../instructions/index.js';
