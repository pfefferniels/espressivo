/**
 * The fifteen prose tables in a shape the applier can walk: one {@link RegistryRow}
 * per live attribute, carrying the scale space it lives in, the input predicate the
 * validation gate applies, whether it feeds the center population, and its P5r verdict.
 *
 * What is not here. The excluded attributes have no row, because a row is a licence to
 ...
 * known statically, and the two level rows' center is a run-time quantity the applier
 * binds. Finding elements, resolving styles and deciding atomic groups is the applier's walk;
 * where the design mandates a handler that is more than "one attribute, one space" — the level pairs,
...
/**
 * the fifteen user-facing dimensions (the v2 set), and the vocabulary the validator
...
/** a missing key means 1, which means identity. */
export type ExaggerationFactors = Partial<Readonly<Record<ExpressionDimension, number>>>;

/**
 * The P5r column: whether the *rendered* effect is monotone in `s`.
 *
 * P5a — `|T(x')| = s·|T(x)|` — is definitional and holds for every row by construction;
...
/** `instruction`, `def`, or `both` — the "site discipline" column, derived from the sites. */
export type SiteKinds = 'instruction' | 'def' | 'both';

/**
 * A row's scale space with everything that is known statically bound.
 *
 * `level` is the one space whose parameter is not: the center is a property of the
 ...
 *
 * `joint-trim` is not a scalar space at all: it transforms the `(lateStart, earlyEnd)`
...
export interface RegistryRow {
  ...
  /**
   * whether a value read at this row's site joins its dimension's center population.
  ...
  /**
   * the input predicate: the attribute's mathematical domain, as the "domain +
  ...
  /** The dimension this row is compiled from, plus the obligation it carries. */
  readonly notes: string;
}

// --- Input predicates (the "domain" column) -------------------------------------
//
// Named rather than inlined so that a row reads as one line, and so that two rows
...
/** `[0,∞)` — `frameLength`, whose setter is a one-sided `Math.max(0,·)`. */
const nonNegative = (x: number): boolean => Number.isFinite(x) && x >= 0;
/** `[0,1]` — curvature; the far bound is an admissible fixed point. */
const unitClosed = (x: number): boolean => Number.isFinite(x) && x >= 0 && x <= 1;
/** `[-1,1]` — protraction; `±1` are admissible boundary fixed points. */
const signedUnitClosed = (x: number): boolean => Number.isFinite(x) && x >= -1 && x <= 1;
/** `(0,1)` — `meanTempoAt`, narrower than the logit space's own closed interval. */
...
const TEMPO_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'holds',
    notes:
      'classified def-lookup-first then parseFloat; transformed in ' +
  ...
  },
  {
    ...
    p5r: 'cliff',
    notes:
      'excluded from the center population, still transformed; under gesture scope ' +
      'the pair scales around its own geomean; pair-collapse guard refuses a write that ' +
      "would make String(to') === String(bpm').",
  },
  {
    dimension: 'tempo',
    attribute: 'value',
    ...
    p5r: 'holds',
    notes:
      'deduped by def identity; a part header shadows the global one WHOLESALE; a ' +
  ...
  },
  {
    ...
    p5r: 'cliff',
    notes:
      'transformed only where @transition.to is present, else inert; a result that ' +
      'saturates onto an exact bound is refused and reported, because 0 and 1 turn the ' +
...
const DYNAMICS_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'saturates',
    notes:
      'clamped into options.velocityRange and counted. Under gesture scope this ' +
      'endpoint moves only as half of a transition pair, around the pair’s own geomean; on a ' +
      'constant instruction it is untouched. @subNoteDynamics is read, never ' +
      'written: it selects a harsher CC-based range regime for the same s.',
  },
  {
    dimension: 'dynamics',
    attribute: 'transition.to',
    ...
    p5r: 'saturates',
    notes:
      'excluded from the center population, still transformed; never materialized and ' +
      'never dropped, since its mere presence is the switch into the renderer’s transition ' +
      'branch; the MEI end-marker duplicate moves with it under gesture scope.',
  },
  {
    dimension: 'dynamics',
    attribute: 'value',
    ...
    p5r: 'saturates',
    notes:
      'the correct lever for name-valued volumes, which are the MEI norm; deduped by ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes:
      'inert on a constant instruction (the renderer force-zeroes it there): reported ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes: 'same inertness rule; ±1 are admissible boundary fixed points.',
  },
];

// --- rubato -------------------------------------------------------------------------
//
// One row per attribute, two sites each. The design gives element and def rows separately
...
const RUBATO_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'holds',
    notes:
      'the gate rejects ≤ 0 rather than repairing it: 0 collapses the frame to an ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes:
      'half of the joint trim t = lateStart + (1 − earlyEnd); the ' +
      'effective window is resolved across def and element FIRST, and a def any of whose ' +
      'referencing elements overrides exactly one bound is excluded from the trim.',
  },
  {
    dimension: 'rubato',
    attribute: 'earlyEnd',
    ...
    p5r: 'holds',
    notes:
      'the other half; written atomically with @lateStart, because 0 ≤ ls < ee ≤ 1 is ' +
...
// element can carry are excluded: three replacements whose neutral lives in the MSM, and two
// pitch attributes — see EXCLUDED_ARTICULATION_LEVERS below, which the applier
...
const ARTICULATION_ROWS: readonly RegistryRow[] = [
  articulationRow(
    ...
    positive,
    'holds',
    'neutral ≡ absent (the serializer omits it at 1.0). On an INLINE <articulation> a ' +
  ...
  articulationRow(
    ...
    positive,
    'non-monotone',
    "affine with @absoluteVelocityChange: rendered v' = v·rˢ + s·c is not monotone in " +
      's, and a pair that cancels at s = 1 is not a fixed point. The net-deviation remedy was ' +
      'rejected (needs MSM velocities); the disposition is disclosure — a report note ' +
  ...
  articulationRow(
    ...
    anyFinite,
    'saturates',
    "signed ticks at the performance PPQ. The renderer's halving loop plateaus for " +
  ...
  articulationRow(
    ...
    anyFinite,
    'cliff',
    'the best-behaved lever at attribute level; the cliff is the shared pass-two ' +
      'commit guard, which discards ALL THREE ms modifiers when an exaggerated value inverts ' +
      'the note. Bounding it needs the rendered note length (MSM), so it is report-only.',
  ),
  articulationRow(
    'absoluteDelay',
    GAIN_SPACE,
    anyFinite,
    'holds',
    'the cleanest lever: it moves both note edges. Large values trigger the map sort ' +
  ...
  articulationRow(
    ...
    anyFinite,
    'cliff',
    'moves the onset but not the end, so it SHORTENS the note; past the remaining ' +
  ...
  articulationRow(
    ...
    anyFinite,
    'non-monotone',
    'the idiomatic accent lever. Never clamped: velocity is a shared bus ' +
      'and the final value depends on MSM note data. Reported as velocity coefficients.',
  ),
];
...
const ACCENTUATION_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'saturates',
    notes:
      'the SINGLE site: the def triple (@value, @transition.from, ' +
...
const ORNAMENT_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'cliff',
    notes:
      'scaled by the SAME factor as @frameLength: the frame [start, start+length] is a ' +
      'geometric pair and scaling the length alone drags the centroid late. Negative values ' +
      'are idiomatic. @time.unit and @noteoff.shift are read, never written. ' +
  ...
  },
  {
    ...
    p5r: 'cliff',
    notes:
      'the v3 spelling of @frame.start, and a v3 structural marker: its mere presence ' +
  ...
  },
  {
    ...
    p5r: 'cliff',
    notes:
      "s ≥ 0 because the setter's one-sided Math.max(0,·) turns a negative value " +
      ...
      'ONE row for both generations — v3 changed the encoding (a unit suffix) and the ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes:
      'the spacing curve of the roll, an exponent rather than a width, which is why it ' +
      'is its own dimension. The epsilon floor is DROPPED: this space cannot produce a ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes:
      'a GAIN, not a logit on [−1,1]: nothing enforces that range ' +
      ...
      'reported informationally, never corrected. The design says v3 "replaces" this row; the code ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes:
      'scaled ONLY where physically present: absent it defaults to @transition.from, ' +
...
const ASYNCHRONY_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'saturates',
    notes:
      'exactly linear in the document; the saturation is render-side and one-sided ' +
...
// therefore in their units (ms / velocity units / ms of note length). The width-like
...
/** the per-distribution groups: element local name → its width-like attributes. */
...
const IMPRECISION_DOMAINS: readonly (readonly [ExpressionDimension, string, string])[] = [
  ...
  [
    'imprecisionDynamics',
    IMPRECISION_MAP_DYNAMICS,
    'velocity units, unclamped in the map and biting after the dynamics pass — a velocity ' +
      'reporting dimension',
  ],
  [
    'imprecisionDuration',
  ...
  ([dimension, map, domainNote]) =>
    IMPRECISION_GROUPS.flatMap(([element, attributes, groupNote]) =>
      attributes.map((attribute): RegistryRow => ({
        ...
        valueDomain: anyFinite,
        p5r: 'holds',
        notes: `${element} — ${groupNote}. Domain: ${domainNote}. s ≥ 0 is an API constraint, not a preference: s < 0 inverts every lower/upper pair, while s = 0 is well defined and means "remove all imprecision".`,
      })),
    ),
);
...
const PEDAL_ROWS: readonly RegistryRow[] = [
  {
    ...
    p5r: 'holds',
    notes:
      'The same Bézier pair as the dynamics shape, and the same cancellation ' +
  ...
  },
  {
    ...
    p5r: 'holds',
    notes:
      'the best-behaved attribute in the family: signed, symmetric, neutral at 0, and ' +
      'notated dates untouched. @position and @transition.to stay excluded under ' +
      'canonical pedal maps are exact 0.0/1.0, where every candidate transform has a pole.',
  },
];

/** The live attributes, in registry order. */
 ...
 * is not a live one — which is the answer for every the exclusion, so the applier asks this
...
/** the "site discipline" column, derived rather than restated. */
 ...
 *
 * `center` is required by, and only by, the two level dimensions.
 */
...
/**
 * a dimension's admissible-s domain is the intersection over its rows' scale
 ...
 * scales both by ONE factor — so the dimension is `s ≥ 0`, which is what the `0 … 4` assumes.
 */
...
/**
 * The attribute names of one imprecision distribution's atomic group, in registry order,
...
/** The distribution element local names the imprecision rows group, in registry order. */
...
/** the imprecision domain nothing in this codebase reads. Reported inert. */
export const INERT_IMPRECISION_MAP = IMPRECISION_MAP_TUNING;

/**
 * the enum that decides whether a `<temporalSpread>` frame is measured in
 * PPQ-relative ticks or in absolute milliseconds.
 *
 * Excluded as an enum, but load-bearing for the FACTOR: the `ornamentSpread` row makes the
...
/**
 * the enum that decides which attribute absorbs the scaled offset, and which can
...
/**
 * the boolean that switches a `<dynamics>` into sub-note CC 7 curve points. Read
 ...
 * path, so the clamp is no guarantee for such an instruction.
 */
 ...
 * lands (the "documented no-ops the report must catch").
 */
export const LOOP_ATTRIBUTE = 'loop';
export const STICK_TO_MEASURES_ATTRIBUTE = 'stickToMeasures';

/**
 * the articulation levers whose neutral is the attribute's own ABSENCE and whose
 * effective neutral lives in the MSM, out of reach.
 *
 * Read, never written: a site carrying one has a component the transform cannot reach,
...
export const EXCLUDED_ARTICULATION_LEVERS: readonly string[] = [
  ...
  'absoluteDurationMs',
  'absoluteVelocity',
  // the two pitch levers are excluded components of the same element, so `partial`'s rule
...
/**
 * on an INLINE `<articulation>` the original duration is read once up front, so the
...
/** The style collection each level domain's defs live in — the def side. */
...
/** The prevailing-level attribute of each level domain — the population, before defs. */
...
/** Where `accentuationPattern@scale`'s velocity estimate reads its anchors. */
