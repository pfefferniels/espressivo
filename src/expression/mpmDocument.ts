 ...
 * `accentuationPatternDef` gains `length="4"`, `GenericMap.parseData` ends in an
 * unconditional `sortXml()` that reorders map children and hoists every instruction in
 ...
 * (XomTypes.ts:381-383). What DOES hold, and is what P1's identity predicate is asserted
 * against, is that the round trip is idempotent after one application:
 ...
 *
 * Validation is XML well-formedness only — not the root element's name, not the namespace,
 * not the presence of a `<performance>`. Structural checks and typed errors belong to the
 * facade, so the navigation modules above this layer can report an empty document as
 * "nothing to transform" rather than having a guess about intent baked in underneath them.
 *
 * @throws {ParsingException} from `Builder`, for both malformed XML and a document with no
 *   root element. W3 turns it into the typed `ParseError`.
 */
