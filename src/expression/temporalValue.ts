 ...
 * the two bounds of one frame can be measured on different clocks.
 *
 ...
 * document generation on the caller's behalf.
 *
 ...
 * {@link resolveTemporalDomain} and used only for the report, never to rewrite a value.
 */
...
/**
 * The three v3 unit suffixes, plus the empty one a suffix-less value carries. `''` is not a
 * fourth unit but the absence of one, which v3's own corpus writes and its base schematron
 * allows (`att.time.frameLength.xml:21`), and whose meaning {@link resolveTemporalDomain}
 * resolves rather than the value itself.
 */
 ...
 * `value` is the bare number as written — `"80%"` is `80`, not `0.8`, since resolving a
 * percentage against a principal note is the renderer's job (PARITY.md §6.2 D4 puts it in the
 * tick domain). `suffix` is the trailing bytes verbatim.
 */
...
/**
 * The v3 schematron's number grammar with its mandatory suffix (`att.time.frame.xml:20`,
 * `temporalSpread.xml:34`), transliterated from `TemporalValue.ts`'s `SUFFIXED`.
 *
 * Every exclusion is load-bearing: no leading-dot form (`.5`), no exponent (`1e3`), no `+`, no
 * `Infinity`/`NaN`, no surrounding whitespace. That makes it narrower than the `parseFloat` the
 * v2 path reads with, and the gap is why the generation has to be detected before a value is
 * read at all: `parseFloat("80%")` is `80`, so a v3 value read through the v2 path scales
 * correctly and then serializes as `"120"`, silently deleting the unit.
 */
 ...
 * `V3_UNIT_SUFFIX`. It ignores whether the rest of the value parses, so `frameLength="abc%"` is
 * a malformed *v3* value rather than a v2 one. Reading it as v2 instead would put it through
 * `parseFloat`, get `NaN` and refuse it as out-of-domain — the same outcome by the wrong route,
 * which stops being harmless the moment the sibling bound is well-formed and would be written
 * on its own.
 */
...
/**
 * Read a value the way a real v3 document writes it: the schematron's number, suffix optional.
 *
...
/**
 * Put a scaled number back under the suffix it was found with. The number goes through
 * {@link numberToString} — the engine's one formatting choke point — and the suffix is
 * concatenated verbatim, so `"80%"` scaled by 1.5 is `"120%"` and `"44"` scaled by 2 is `"88"`,
 * where the renderer's own `formatTemporalValue` would write `"88ticks"`.
 */
 ...
 *
 * - Detection is per ELEMENT, not per document. A performance may hold a v2 and a v3 spread
 *   side by side, and each keeps its own reading and its own byte discipline.
 * - Any marker makes the whole element v3, including the mixed spelling
 *   `frame.start="-22.0" frameLength="44%"`; a per-attribute generation would have to write
 *   half a v2 and half a v3 element.
 * - `alignment` is v3-only but deliberately NOT a marker: a `<temporalSpread>` carrying nothing
 *   but a reference-implementation-style `alignment` is otherwise pure v2 and must keep
 *   reading, and re-serializing, as v2.
 */
...
/**
 * Which attribute physically carries the frame's offset on a v3 spread, or null when it carries
 * none. `frame.offset` first, then `frame.start` as the legacy alias the v3 reader still accepts
