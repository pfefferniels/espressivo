...
function createPerformance(name: string, pulsesPerQuarter?: number): IPerformance | null {
    ...
    const idAttr = new Attribute("xml:id", "http://www.w3.org/XML/1998/namespace", "meico_" + uuidv4());
 ...
 */
function createPart(name: string, number: number, midiChannel: number, midiPort: number, id?: string): IPart | null {
 ...
 */
function addMetadataToMpm(mpm: Mpm, author: IAuthor | null, comment: IComment | null, relatedResources: IRelatedResource[] | null): boolean {
...
function getProgramChangeFromInstrumentName(name: string): number {
    ...
    const dict: Map<string, number> = new Map([
        ["piano", 0], ["acoustic grand piano", 0], ["bright acoustic piano", 1],
        ["electric grand piano", 2], ["honky-tonk piano", 3],
        ["electric piano 1", 4], ["electric piano 2", 5],
        ["harpsichord", 6], ["clavinet", 7], ["clavi", 7],
        ["celesta", 8], ["glockenspiel", 9], ["music box", 10],
        ["vibraphone", 11], ["marimba", 12], ["xylophone", 13],
        ["tubular bells", 14], ["dulcimer", 15],
        ["drawbar organ", 16], ["organ", 19], ["church organ", 19],
        ["reed organ", 20], ["accordion", 21], ["harmonica", 22],
        ["tango accordion", 23], ["bandoneon", 23],
        ["acoustic guitar (nylon)", 24], ["guitar", 24], ["nylon guitar", 24],
        ...
        ["sitar", 104], ["banjo", 105], ["shamisen", 106],
        ["koto", 107], ["kalimba", 108], ["bag pipe", 109], ["bagpipe", 109],
...
export class MusicXml2MsmMpmConverter {
    ...
    convert(musicXml: MusicXml | null): KeyValue<Msm | null, Mpm | null> {
        ...
        if (this.musicXml.getFile() !== null) {
            ...
            relatedResources.push(createRelatedResource(getFileName(this.musicXml.getFile()!), "musicxml")!);
            relatedResources.push(createRelatedResource(getFileName(this.msm.getFile()!), "msm")!);
            const comment = createComment("This MPM has been generated from '" + getFileName(this.musicXml.getFile()!) + "' using the meico MEI converter v" + Meico.version + ".", null);
            addMetadataToMpm(this.mpm, createAuthor("meico", null, null), comment, relatedResources);
        } else {
            const comment = createComment("This MPM has been generated from MEI code using the meico MEI converter v" + Meico.version + ".", null);
        ...
        // cleanup
        if (this.cleanup)
            msmCleanup(this.msm);                                                                                         // cleanup of the msm objects to remove all conversion related and no longer needed entries in the msm objects

        console.log("MusicXML to MSM and MPM conversion finished. Time consumed: " + (Date.now() - startTime) + " milliseconds");
    ...
    private processPartList(): void {
        ...
        const groups: Map<number, Element> = new Map();                              // group number, part-group element
        let number: number = 0;                                                      // default initial value
        let midiChannel: number = 0;                                                 // default initial value
        let midiPort: number = 0;                                                    // default initial value
        ...
        for (let ci = 0; ci < children.size(); ci++) {
            ...
            let foundProgramChange: boolean = false;
            let midiInstrNum: number = 0;
