...
describe('Mei – addIds', () => {
    ...
    it('should generate ids with the meico prefix', () => {
        ...
        const note = mei.getRootElement()!.query("descendant::*[local-name()='note']").get(0) as unknown as Element;
        expect(note.getAttribute('id', 'http://www.w3.org/XML/1998/namespace')!.getValue()).toMatch(/^meico_/);
        logSpy.mockRestore();
    });
    ...
    it('should leave existing ids alone', () => {
        ...
        expect(first.getAttribute('id', 'http://www.w3.org/XML/1998/namespace')!.getValue()).toBe('keepme');
    ...
    it('should not touch elements outside the id-worthy set', () => {
        ...
        const layer = mei.getRootElement()!.query("descendant::*[local-name()='layer']").get(0) as unknown as Element;
...
describe('Mei – removeRendElements', () => {
    ...
    it('should handle several rends and nested content', () => {
        ...
        const dir = mei.getMusic()!.query("descendant::*[local-name()='dir']").get(0) as unknown as Element;
...
<mei xmlns="http://www.music-encoding.org/ns/mei"><meiHead/></mei>`;
        ...
        expect(() => (mei as unknown as { removeRendElements(): void }).removeRendElements()).not.toThrow();
        logSpy.mockRestore();
    });
});
...
describe('Mei – resolveCopyofs', () => {
    it('should replace a copyof placeholder by a copy of its target', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const mei = new Mei(wrap('<note xml:id="n1" dur="4" pname="c" oct="4"/><note xml:id="n2" copyof="#n1"/>'), true);
    ...
    it('should keep the placeholder id on the inserted copy', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const mei = new Mei(wrap('<note xml:id="n1" dur="4" pname="c"/><note xml:id="n2" copyof="#n1"/>'), true);

        mei.resolveCopyofs();

        const second = mei.getMusic()!.query("descendant::*[local-name()='note']").get(1) as unknown as Element;
        expect(second.getAttribute('id', 'http://www.w3.org/XML/1998/namespace')!.getValue()).toBe('n2');
        logSpy.mockRestore();
    });

    it('should resolve sameas just like copyof', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const mei = new Mei(wrap('<note xml:id="n1" dur="8" pname="d"/><note xml:id="n2" sameas="#n1"/>'), true);
    ...
    it('should report and drop a placeholder whose target does not exist', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const mei = new Mei(wrap('<note xml:id="n1" dur="4"/><note xml:id="n2" copyof="#missing"/>'), true);
    ...
    it('should detect a circular reference and give up', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
        const mei = new Mei(wrap('<note xml:id="n1" copyof="#n2"/><note xml:id="n2" copyof="#n1"/>'), true);
    ...
    it('should give the copied descendants fresh ids', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const mei = new Mei(wrap('<chord xml:id="c1"><note xml:id="n1" dur="4"/></chord><chord xml:id="c2" copyof="#c1"/>'), true);

        mei.resolveCopyofs();

        const ids = mei.getMusic()!.query("descendant::*[local-name()='note']");
        const original = (ids.get(0) as unknown as Element).getAttribute('id', 'http://www.w3.org/XML/1998/namespace')!.getValue();
        const copied = (ids.get(1) as unknown as Element).getAttribute('id', 'http://www.w3.org/XML/1998/namespace')!.getValue();
    ...
    it('resolveCopyofsAndSameas should be the same operation', () => {
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
        const mei = new Mei(wrap('<note xml:id="n1" dur="4" pname="e"/><note xml:id="n2" copyof="#n1"/>'), true);
...
<mei xmlns="http://www.music-encoding.org/ns/mei">
  ...
  <music><body><mdiv><score>
    ...
    const sectionOrder = (mei: Mei): string[] => {
        const top = mei.getMusic()!.query("descendant::*[local-name()='section' and @xml:id='top']").get(0) as unknown as Element;
        const order: string[] = [];
    ...
    it('should drop siblings that the plist does not mention', () => {
        ...
        expect(mei.getMusic()!.query("descendant::*[local-name()='section' and @xml:id='a']").size()).toBe(0);
        expect(mei.getMusic()!.query("descendant::*[local-name()='section' and @xml:id='b']").size()).toBe(1);
        logSpy.mockRestore();
    });
});
