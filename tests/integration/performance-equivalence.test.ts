...
function extractNoteStructure(xml: string): Array<{pitch: number, date: number, duration: number, velocity?: number, msDate?: number, msDateEnd?: number}> {
    const notes: Array<{pitch: number, date: number, duration: number, velocity?: number, msDate?: number, msDateEnd?: number}> = [];
    ...
    while ((match = noteRegex.exec(xml)) !== null) {
        ...
        notes.push({
            ...
            msDate: getAttr('milliseconds.date') !== undefined ? normalizeNum(getAttr('milliseconds.date')!) : undefined,
            msDateEnd: getAttr('milliseconds.date.end') !== undefined ? normalizeNum(getAttr('milliseconds.date.end')!) : undefined,
        });
    }
    return notes;
}
 ...
 */
function compareMidiStructure(tsMidi: Midi, refMidiBytes: Uint8Array): {
    tracksMatch: boolean;
            ...
            it('should produce milliseconds.date values close to Java reference', () => {
                ...
                for (let i = 0; i < Math.min(tsNotes.length, refNotes.length); i++) {
                    if (tsNotes[i].msDate && refNotes[i].msDate) {
                        const tsMs = parseFloat(tsNotes[i].msDate!);
                        const refMs = parseFloat(refNotes[i].msDate!);
                    ...
                    if (tsNotes[i].msDateEnd && refNotes[i].msDateEnd) {
                        const tsMs = parseFloat(tsNotes[i].msDateEnd!);
                        const refMs = parseFloat(refNotes[i].msDateEnd!);
                        expect(Math.abs(tsMs - refMs)).toBeLessThan(1.0);
                    }
                }
            });
            ...
            it('should produce velocity values close to Java reference', () => {
                ...
                for (let i = 0; i < Math.min(tsNotes.length, refNotes.length); i++) {
                    if (tsNotes[i].velocity && refNotes[i].velocity) {
                        const tsVel = parseFloat(tsNotes[i].velocity!);
                        const refVel = parseFloat(refNotes[i].velocity!);
