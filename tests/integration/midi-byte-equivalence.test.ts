        ...
        for (let i = 0; i < track.size(); i++) {
            ...
            if (msg instanceof ShortMessage) {
                ...
                switch (msg.getCommand()) {
                    case ShortMessage.NOTE_ON: info.type = 'noteOn'; break;
                    case ShortMessage.NOTE_OFF: info.type = 'noteOff'; break;
                    case ShortMessage.CONTROL_CHANGE: info.type = 'controlChange'; break;
                    case ShortMessage.PROGRAM_CHANGE: info.type = 'programChange'; break;
                    default: info.type = 'shortMessage';
                }
    ...
    for (let t = 0; t < tsEvents.length; t++) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (te.type === 'noteOn' || te.type === 'noteOff') {
                if (te.channel !== re.channel) diffs.push(`Track ${t} event ${i}: channel TS=${te.channel} vs Java=${re.channel}`);
                if (te.data1 !== re.data1) diffs.push(`Track ${t} event ${i}: pitch TS=${te.data1} vs Java=${re.data1}`);
                if (te.type === 'noteOn' && te.data2 !== re.data2) diffs.push(`Track ${t} event ${i}: velocity TS=${te.data2} vs Java=${re.data2}`);
            } else if (te.type === 'controlChange') {
                if (te.data1 !== re.data1) diffs.push(`Track ${t} event ${i}: CC# TS=${te.data1} vs Java=${re.data1}`);
                if (te.data2 !== re.data2) diffs.push(`Track ${t} event ${i}: CC value TS=${te.data2} vs Java=${re.data2}`);
            } else if (te.type === 'meta') {
                if (te.metaType !== re.metaType) diffs.push(`Track ${t} event ${i}: metaType TS=${te.metaType} vs Java=${re.metaType}`);
            }
        }
    }
...
const meiFigures = readdirSync(MEI_DIR).filter(f => f.endsWith('.mei')).map(f => f.replace(/\.mei$/, '')).sort();
...
const deterministicMaps = ['rubato', 'asynchrony', 'metrical_accentuation', 'movement', 'ornamentation'];
