 ...
 * immutable: those suites compare this port's output against meico's,
 ...
 *
 * Every number asserted here is computed by hand in the comment above it, from
 * the worked vectors and these three rules, never read off the implementation:
 *
 ...
 * | fixture | pins |
 * |---|---|
 * | `turn-atstart` | the figure-1 turn, `%` frame, monophonic note-offs |
 * | `turn-atend` | the same figure aligned at the end, and its head leftover |
 * | `trill-repetitions` | repeat-group expansion, mixed `ticks`/`%` frame, the `pass` provenance |
 * | `spread-ms` | a millisecond frame feeds the unchanged v2 marker engine |
...
// ---------------------------------------------------------------------------------------
// the figure-1 turn, aligned at the start
// ---------------------------------------------------------------------------------------

describe('turn-atstart: the figure-1 turn', () => {
  /**
...
// ---------------------------------------------------------------------------------------
// the same figure, aligned at the end
// ---------------------------------------------------------------------------------------

describe('turn-atend: the figure-2 turn', () => {
  /**
   ...
   * end 1440 ⇒ 240, 240, 240, and 1440 − 1440 = 0. The zero is what the own
  ...
  it(
    ...
    () => {
      ...
      // so — `ornamented` covers "generated by or altered by" an ornament. It gets
      // `ornament.carved` plus the ref, and none of
  ...
  it(
    ...
    () => {
      // The D10 id-uniqueness ruling (): the id
...
describe('trill-repetitions: the figure-3 trill', () => {
  /**
...
// ---------------------------------------------------------------------------------------
// a millisecond frame feeds the v2 marker engine
// ---------------------------------------------------------------------------------------

describe('spread-ms: a millisecond frame over a generated chord', () => {
  /**
...
describe('diatonic-key: scale steps resolved against the key signature (D8)', () => {
   ...
   * slots at 0, 240, 480, 720 with durations 240, 240, 240, 720.
   */
  it(
    ...
    () => {
      ...
      // to "some pitches": 73 ≠ 72 and 78 ≠ 77
...
describe('v2-passthrough: a v2 ornamentation document is untouched by v3 (D6)', () => {
   ...
   * noteoff.shift absent). Nothing in the document is v3, so 's gate must keep
...
describe('a negative intensity reaches neither the augmented MSM nor the MIDI export', () => {
   ...
   * and a tenth pair would commit a document nobody would ever author.
