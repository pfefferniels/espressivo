 ...
 */
function buildMap(name: string, entries: Array<[string, number | null]>): Element {
...
describe('AbstractMsm', () => {
    ...
    describe('getElementAtAfter', () => {
        it('should return the first element at the exact date', () => {
            const map = buildMap('score', [['note', 0], ['note', 720], ['note', 1440]]);
        ...
        it('should return the first element for date 0', () => {
            const map = buildMap('score', [['note', 0], ['note', 720]]);
        ...
        it('should skip elements without a date attribute', () => {
            const map = buildMap('score', [['note', null], ['note', 720]]);
        ...
        it('should search across all element names when no name is given', () => {
            const map = buildMap('score', [['rest', 0], ['note', 720]]);
    ...
    describe('getElementAtAfterByName', () => {
        it('should only consider elements of the given name', () => {
            const map = buildMap('score', [['rest', 0], ['note', 720], ['rest', 1440]]);
        ...
        it('should return null when no element of that name qualifies', () => {
            const map = buildMap('score', [['rest', 0], ['rest', 720]]);
    ...
    describe('getElementBeforeAt', () => {
        ...
        it('should return null when every element is after the date', () => {
            const map = buildMap('score', [['note', 720], ['note', 1440]]);
        ...
        it('should skip elements without a date attribute', () => {
            const map = buildMap('score', [['note', 0], ['note', null]]);
    ...
    describe('getElementBeforeAtByName', () => {
        it('should only consider elements of the given name', () => {
            const map = buildMap('score', [['note', 0], ['rest', 720], ['rest', 1440]]);
        ...
        it('should treat an empty name as "any element"', () => {
            const map = buildMap('score', [['note', 0], ['rest', 720]]);
    ...
    describe('deleteEmptyMaps', () => {
        ...
        it('should remove maps that have no children', () => {
            const msm = new Msm('<msm title="T" pulsesPerQuarter="720"><global><header/><dated>'
        ...
        it('should not touch non-map elements such as score', () => {
            const msm = new Msm('<msm title="T" pulsesPerQuarter="720"><global><header/><dated/></global>'
        ...
        it('should remove empty maps in parts as well as in global', () => {
            const msm = new Msm('<msm title="T" pulsesPerQuarter="720"><global><header/><dated><markerMap/></dated></global>'
                + '<part name="P" number="1" midi.channel="0" midi.port="0"><header/><dated>'
        ...
        it('should treat a map holding only whitespace text as non-empty', () => {
            ...
            const msm = new Msm('<msm title="T" pulsesPerQuarter="720"><global><header/><dated><markerMap>\n  </markerMap></dated></global></msm>');
            msm.deleteEmptyMaps();

            expect(msm.getGlobal()!.getFirstChildElement('dated')!.getFirstChildElement('markerMap')).not.toBeNull();
        });
    });
});
