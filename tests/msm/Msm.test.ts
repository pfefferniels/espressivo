...
function addNote(score: Element, date: number, duration: number, pitch: number, extra: Record<string, string> = {}): Element {
...
function msmWithNotes(ppq: number, notes: Array<[number, number, number]>): Msm {
...
describe('Msm', () => {
    ...
    describe('createMsm', () => {
        ...
        it('should use a provided id when given', () => {
            ...
            const idAttr = msm.getRootElement()!.getAttribute('id', 'http://www.w3.org/XML/1998/namespace');
    ...
    describe('deleteEmptyMaps', () => {
        ...
        it('should identify empty maps via XPath query', () => {
            ...
            const dated = msm.getRootElement()!.getFirstChildElement('global')!.getFirstChildElement('dated')!;
            expect(dated.getFirstChildElement('keySignatureMap')).not.toBeNull();
    ...
    describe('convertPPQ', () => {
        it('should scale date, date.end and duration when doubling the resolution', () => {
            const msm = msmWithNotes(360, [[0, 360, 60], [360, 180, 62]]);
            msm.convertPPQ(720);

            expect(msm.getPPQ()).toBe(720);
            const notes = msm.getParts().get(0).getFirstChildElement('dated')!.getFirstChildElement('score')!.getChildElements('note');
        ...
        it('should halve the values when the resolution is halved', () => {
            ...
            const note = msm.getParts().get(0).getFirstChildElement('dated')!.getFirstChildElement('score')!.getChildElements('note').get(0);
    ...
    describe('getMinimalPPQ', () => {
        it('should return 1 when everything is aligned to whole quarter notes', () => {
            expect(msmWithNotes(720, [[0, 720, 60], [720, 720, 62]]).getMinimalPPQ()).toBe(1);
        });

        it('should return 2 when eighth notes are involved', () => {
            expect(msmWithNotes(720, [[0, 360, 60], [360, 360, 62]]).getMinimalPPQ()).toBe(2);
        });

        it('should return 4 when sixteenth notes are involved', () => {
            expect(msmWithNotes(720, [[0, 180, 60], [180, 180, 62]]).getMinimalPPQ()).toBe(4);
        });

        it('should take the finest subdivision found across all notes', () => {
            expect(msmWithNotes(720, [[0, 720, 60], [720, 180, 62], [900, 360, 64]]).getMinimalPPQ()).toBe(4);
        });
    ...
    describe('getPart', () => {
        ...
        it('should find a part by its number', () => {
            expect(threePartMsm().getPart(2, 'nonexistent', 99, 99)!.getAttributeValue('name')).toBe('Violin');
        });
        ...
        it('should fall back to the midi port and channel when neither number nor name matches', () => {
            expect(threePartMsm().getPart(99, 'nonexistent', 2, 1)!.getAttributeValue('name')).toBe('Cello');
        });
    ...
    describe('removeRests', () => {
        ...
        it('should be a no-op when there are no rests', () => {
            ...
            expect(msm.getParts().get(0).getFirstChildElement('dated')!.getFirstChildElement('score')!.getChildElements('note').size()).toBe(1);
        });
    });
    ...
    describe('getEndDate', () => {
        it('should return the latest note offset', () => {
            expect(msmWithNotes(720, [[0, 720, 60], [720, 1440, 62]]).getEndDate()).toBe(2160);
        });

        it('should consider a long early note that outlasts later ones', () => {
            expect(msmWithNotes(720, [[0, 5760, 60], [720, 720, 62]]).getEndDate()).toBe(5760);
        });
    ...
    describe('addIds', () => {
        ...
        it('should generate unique ids', () => {
            const msm = msmWithNotes(720, [[0, 720, 60], [720, 720, 62], [1440, 720, 64]]);
            msm.addIds();

            const notes = msm.getParts().get(0).getFirstChildElement('dated')!.getFirstChildElement('score')!.getChildElements('note');
            const generated = new Set<string>();
            for (let i = 0; i < notes.size(); ++i) generated.add(notes.get(i).getAttribute('id', XML_NS)!.getValue());
    ...
    describe('exportMidi', () => {
        ...
        it('should emit a note on and a note off per note, at date and date+duration', () => {
            const seq = msmWithNotes(720, [[0, 720, 60], [720, 360, 64]]).exportMidi()!.getSequence();
        ...
        it('should use velocity 100 for a plain (non-expressive) export', () => {
            const seq = msmWithNotes(720, [[0, 720, 60]]).exportMidi()!.getSequence();
        ...
        it('should write a track name for a named part', () => {
            ...
            expect(new TextDecoder().decode((names[0].getMessage() as MetaMessage).getData())).toBe('Piano');
        });
        ...
        it('should fall back to a piano program change for an unnamed part', () => {
            ...
            expect((pcs[0].getMessage() as ShortMessage).getData1()).toBe(EventMaker.PC_Acoustic_Grand_Piano);
        ...
        it('should render markers from the global markerMap', () => {
            const msm = msmWithNotes(720, [[0, 720, 60]]);
            const markerMap = msm.getGlobal()!.getFirstChildElement('dated')!.getFirstChildElement('markerMap')!;
            ...
            expect(new TextDecoder().decode((markers[0].getMessage() as MetaMessage).getData())).toBe('Rehearsal A');
        });

        it('should render time and key signatures from the global maps', () => {
            const msm = msmWithNotes(720, [[0, 720, 60]]);
            const dated = msm.getGlobal()!.getFirstChildElement('dated')!;
            dated.getFirstChildElement('timeSignatureMap')!.appendChild(Msm.makeTimeSignature(0, 3, 4, null));
    ...
    describe('exportExpressiveMidi', () => {
        ...
        it('should take the note velocities from the score', () => {
            ...
            expect(shortMessages(seq, NOTE_ON).map(e => (e.getMessage() as ShortMessage).getData2())).toEqual([64, 96]);
        });
        ...
        it('should emit a default channel volume when the part has no channelVolumeMap', () => {
            const seq = performedMsm([100]).exportExpressiveMidi()!.getSequence();
            const volumes = shortMessages(seq, CONTROL_CHANGE)
        ...
        it('should render a channelVolumeMap', () => {
            ...
            for (const [ms, value] of [[0, 80], [500, 110]] as Array<[number, number]>) {
            ...
            const volumes = shortMessages(msm.exportExpressiveMidi()!.getSequence(), CONTROL_CHANGE)
                .filter(e => (e.getMessage() as ShortMessage).getData1() === EventMaker.CC_Channel_Volume);

            expect(volumes.map(e => (e.getMessage() as ShortMessage).getData2()).sort((a, b) => a - b)).toEqual([80, 110]);
        });
        ...
        it('should thin out channel volume events that are closer than the density limit', () => {
            ...
            for (const [ms, value] of [[0, 70], [495, 80], [498, 90], [500, 100]] as Array<[number, number]>) {
                const v = new Element('volume');
                v.addAttribute(new Attribute('date', String(ms)));
                v.addAttribute(new Attribute('milliseconds.date', String(ms)));
                v.addAttribute(new Attribute('value', String(value)));
            ...
            expect(volumes.map(e => [e.getTick(), (e.getMessage() as ShortMessage).getData2()]))
                .toEqual([[0, 70], [500, 100]]);
        });
        ...
        it('should keep a mandatory channel volume event regardless of density', () => {
            ...
            const dated = msm.getParts().get(0).getFirstChildElement('dated')!;
            const cvMap = new Element('channelVolumeMap');
            for (const [ms, value, mandatory] of [[500, 100, false], [498, 90, true]] as Array<[number, number, boolean]>) {
                const v = new Element('volume');
                v.addAttribute(new Attribute('date', String(ms)));
                v.addAttribute(new Attribute('milliseconds.date', String(ms)));
                v.addAttribute(new Attribute('value', String(value)));
        ...
        it('should map positionMap controllers to damper and soft pedal', () => {
            ...
            for (const [controller, value] of [['sustain', 127], ['soft', 64]] as Array<[string, number]>) {
            ...
            const damper = ccs.find(e => (e.getMessage() as ShortMessage).getData1() === EventMaker.CC_Damper_Pedal);
            const soft = ccs.find(e => (e.getMessage() as ShortMessage).getData1() === EventMaker.CC_Soft_Pedal);
