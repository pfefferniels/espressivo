...
function element(name: string, attributes: Record<string, string> = {}, children: (Element | Text)[] = []): Element {
...
describe('RelatedResource', () => {
    ...
    it('returns null when the type is missing, mirroring the null check in Java', () => {
        expect(RelatedResource.createRelatedResource('a.mei', undefined as unknown as string)).toBeNull();
    });

    it('returns null for a null element', () => {
        expect(quiet(() => RelatedResource.createRelatedResource(null as unknown as Element))).toBeNull();
    });
});
...
describe('Metadata', () => {
    describe('factories', () => {
        ...
        it('creates metadata from a list of related resources', () => {
            ...
            expect(m.getXml()!.getFirstChildElement('relatedResources', Mpm.MPM_NAMESPACE)).not.toBeNull();
        });
