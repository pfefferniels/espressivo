...
describe('Performance', () => {
    ...
    describe('perform', () => {
        ...
        it('should give every note a velocity when there is no dynamicsMap', () => {
            ...
            for (const note of scoreNotes(p.perform(msm)))
                expect(num(note, 'velocity')).toBe(100);
        });
        ...
        it('should add performance timing attributes to the global maps', () => {
            ...
            const performedMarker = performed.getGlobal()!.getFirstChildElement('dated')!
                .getFirstChildElement('markerMap')!.getChildElements('marker').get(0);
        ...
        describe('global ornamentationMap', () => {
            ...
            it('should exclude parts that bring their own ornamentationMap from the global one', () => {
                ...
                const pianoNotes = performed.getParts().get(0).getFirstChildElement('dated')!
                    .getFirstChildElement('score')!.getChildElements('note');
                const violinNotes = performed.getParts().get(1).getFirstChildElement('dated')!
                    .getFirstChildElement('score')!.getChildElements('note');
