...
describe('MPM v3 ornament instantiation', () => {
  ...
  describe('worked vector 2: figure-2 turn at end (DESIGN.md §5.2)', () => {
    ...
    /**
     * The leftover is the one note an ornament alters without generating, and D15's facade
     * contract ("generated by or altered by an ornament") can only see what the document says.
     *
     * It gets two attributes and deliberately not the other four: `ornament.carved` (a
     * separate name from `generated` because the two are opposites — this note was in the
     * score and stayed) and `ornament.ref`. No `source`/`slot`/`pass`, because the leftover
     * occupies no position in the expanded sequence; no `anchor`, because the leftover is the
     * anchor — it keeps the principal's own `xml:id`.
     */
     ...
     *
     * Nothing else in this suite constrains the heir when a leftover survives, so a renderer
     * putting the id on both passes everywhere but here.
     */
  ...
  describe('worked vector 3: figure-3 trill (DESIGN.md §5.3)', () => {
    ...
    /**
     * The provenance family over the same eight notes. Every slot of this figure comes from
     * inside the repeat group, so all eight carry a pass:
  ...
  describe('millisecond frame aligned "at end" (D5 amendment)', () => {
    ...
    it('says out loud that the principal’s head is dropped', () => {
      ...
      // frameLength − frame.offset = 60 − (−30) = 90; the intensity-0 case below is where the
      // two part company.
      const logged: string[] = [];
    ...
    it('names the span the spread really produces, not the frame’s length (W9)', () => {
       ...
       * survives — not the 90 ms that `frameLength − frame.offset` claims, which overstates
       * by exactly one frameLength.
       */
      const logged: string[] = [];
      const spy = vi.spyOn(console, 'error').mockImplementation((...args: unknown[]) => {
        logged.push(args.map(String).join(' '));
  ...
  describe('principal resolution (D7)', () => {
    ...
    it('lays out every principal group BEFORE any principal-less ornament', () => {
       ...
       *
       * Measured: swapping the two phases leaves tests/mpm, tests/ornamentation and
       * tests/integration green apart from this one. Orphans appear in only two other tests,
       * and neither has a parented ornament to be ordered against.
       */
      const score = makeScore([makeNote('a', 0, 60)]);
      const map = makeMap([
        makeDef('fig', {
      ...
      // which is what makes the assertion below about the phases rather than about the order
      // the ornaments were encountered in.
      map.addOrnamentV3({
        date: 0,
        nameRef: 'fig',
        noteOrder: '#n1',
  ...
  describe('negative dates (D14)', () => {
    ...
    /**
     * Ruling 5: a dropped note leaves a gap in `ornament.slot` rather than renumbering the
     * notes after it. The attribute is provenance about the expansion, and the
     ...
     *
     * On the shape of the dropped run: for every non-negative intensity the spacing offsets
     * are non-decreasing in i and every noteoff.shift mode keeps the note-off non-decreasing
     ...
     * intensity, where the finiteness guard also drops slot 0.
     */
  ...
  describe('non-finite positions are dropped, not written into the score (W9)', () => {
    /**
     * The construction: intensity −1, `frame.offset="-1000ticks"`,
     ...
     * then computes `duration = end − date = Infinity − Infinity = NaN`, so an unguarded
     * renderer emits a real `<note date="Infinity" duration="NaN">` into the augmented MSM and
     * on into the MIDI export. In MPM v2 the same input could only ever write a marker
     * attribute onto an existing note; v3 turns positions into elements.
     *
  ...
  describe('repetitions="-1" outside the millisecond domain (W9)', () => {
    ...
    it('fills a millisecond frame, which is the case the sentinel exists for', () => {
      ...
      // appends one more principal-pitch slot, because the group opens on the principal's own
      // pitch — so the figure is five notes, 64/66/64/66/64, one over the budget. That
      // overshoot is deliberate and matches the reference, which also appends its landing copy
      // after the fill loop: the budget bounds the repetition, not the landing.
  ...
  describe('provenance attributes on generated notes', () => {
    const renderTrill = () => {
      ...
      // principal's pitch), and dedup collapses nothing — 64 then 65 are distinct.
      const score = makeScore([makeNote('P', 0, 64)]);
    ...
    it('does not let a second ornament inherit the first one’s provenance', () => {
      ...
      //
      // `ornament.carved` is in the stale set too. Within a single render it cannot be on a
      // principal — createChords copies before carve marks — but a principal read back from an
      // already augmented MSM (the second performance of one document, the same scenario the
      // `milliseconds.*` entries exist for) does arrive carrying the mark, and without the
      // entry every note generated from it would claim to be a carved head.
