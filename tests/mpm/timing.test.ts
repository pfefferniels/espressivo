...
} from '../../src/mpm/timing.js';

const QUARTER = 720;
const START = 2160;

/**
 * The resolution every crossing here is asked at. `src/mpm/ppq.js` says why the argument exists
 * and why there is no constant: a document states its own `@pulsesPerQuarter`, and 720 is this
 * test's document rather than the library's.
 */
const PPQ = QUARTER;
...
const SHAPES: [string, TempoWithEndDate][] = [
  ...
  [
    '@meanTempoAt="0", which resolves to a constant at the target',
    span({ transitionTo: 90, meanTempoAt: 0 }),
  ],
  [
    'a @transition.to with no @meanTempoAt, which resolves to a linear ramp',
    span({ transitionTo: 90 }),
  ],
];
 ...
 *
 * The negative ones are not exotic. A caller placing recorded onsets asks
 * `dateAtMilliseconds(onsetMs - startMs)`, and a note sounding ahead of its predecessor makes
 * that negative — which is what an arpeggio and an asynchrony *are*. The ones past 1 are the
 * release of a note held past the last modelled moment of the piece, and the frame of a roll
 * near the final beat.
 */
 ...
 * `src/mpm/timing.ts`; asserting against the looser of them rather than against a digit count
...
describe('dateAtMilliseconds inverts millisecondsAt', () => {
  /**
   * The property the whole module rests on, and the one nothing asserted while the routine this
   * replaced was getting it wrong by 57 000 ticks.
   *
  ...
  test.each(SHAPES)('over %s, for times inside and outside the span', (_, tempo) => {
    ...
    const spanMs = millisecondsAt(resolved.endDate, resolved, PPQ);

    for (const fraction of FRACTIONS) {
      const target = fraction * spanMs;
      const date = dateAtMilliseconds(target, resolved, PPQ);

      expect(Number.isFinite(date), `${target} ms gave ${date}`).toBe(true);
      expect(Math.abs(millisecondsAt(date, resolved, PPQ) - target)).toBeLessThan(TOLERANCE_MS);
    }
  });
  ...
  test('a time before the span is placed at the tempo the span departs from', () => {
    for (const meanTempoAt of [0.3, 0.5, 0.75]) {
      ...
      expect(dateAtMilliseconds(-200, resolved, PPQ)).toBeCloseTo(START - 144, 9);
    }
  });
   ...
   * resolution. The routine this replaced used to read the attribute straight off the record.
   */
  test('a time after the span is placed at the tempo the span arrives at', () => {
    ...
    expect(dateAtMilliseconds(spanMs + 200, resolved, PPQ)).toBeCloseTo(resolved.endDate + 288, 6);
  });

  /**
   * Below about 4 bpm per beat unit the old step — `guess += 0.1 * (targetMs - guessedMs)`,
   * milliseconds added to a tick count — has a multiplier above 2 in absolute value and walks
  ...
  test('a tempo slow enough to make the old fixed step diverge', () => {
    ...
    const date = dateAtMilliseconds(5000, resolved, PPQ);
    expect(millisecondsAt(date, resolved, PPQ)).toBeCloseTo(5000, 6);
  });

  /**
   * A caller that does not know the time it is asking about gets `NaN`, not a tick that looks
   * like an answer. This is the half that says *how* it should fail: the old loop condition
   * `Math.abs(NaN - target) > 1` is `false`, so an iteration that had computed nothing exited as
   * though it had converged and returned its untouched initial guess.
   */
  test('an unknown time gives NaN rather than a plausible tick', () => {
    ...
    expect(dateAtMilliseconds(NaN, resolved, PPQ)).toBeNaN();
    expect(dateAtMilliseconds(Infinity, resolved, PPQ)).toBeNaN();
  });
});

describe('millisecondsAt is defined everywhere and increases', () => {
  /**
   * `Math.pow(negative, non-integer)` is `NaN`, and `NaN` is not caught by any comparison, so it
   * travels until something writes it to the document. An integer exponent — which is exactly
   * `@meanTempoAt="0.5"`, the commonest value written — hides it, and hands back a *positive*
   * elapsed time for a date before the span instead, because the sign of Simpson's `resultConst`
   * flips there.
   */
  test.each(SHAPES)('over %s, on both sides of both boundaries', (_, tempo) => {
    ...
    for (const date of dates) {
      const ms = millisecondsAt(date, resolved, PPQ);
    ...
    expect(millisecondsAt(startDate, resolved, PPQ)).toBe(0);
    expect(millisecondsAt(startDate - 1, resolved, PPQ)).toBeLessThan(0);
  });
  ...
  test.each(SHAPES)('continuously, over %s', (_, tempo) => {
    ...
    expect(Math.abs(millisecondsAt(resolved.startDate - 1e-9, resolved, PPQ))).toBeLessThan(
      TOLERANCE_MS,
    );
    expect(Math.abs(millisecondsAt(resolved.endDate + 1e-9, resolved, PPQ) - spanMs)).toBeLessThan(
      TOLERANCE_MS,
    );
  });
});
...
/**
 * The clamp `bugs.md` #7 asks meico for in `TempoMap.renderTempoToMap`. Doing both keeps the two
 * in step.
 */
