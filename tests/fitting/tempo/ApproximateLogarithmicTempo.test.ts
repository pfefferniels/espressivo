...
import {
  ...
  type SilentOnset,
} from '../../../src/fitting/transformers/tempo/ApproximateLogarithmicTempo.js';
import {
...
function fitAndGetTempos(
  ...
  return getInstructions(mpm, 'tempo', 'global').sort((a, b) => a.date - b.date);
}
...
describe('ApproximateLogarithmicTempo', () => {
  ...
  test('linear ritardando 120 → 80 BPM', () => {
    const totalTicks = 4 * BEAT;
    const onsets = generateOnsets((d) => 120 - 40 * (d / totalTicks), 4);
    const tempos = fitAndGetTempos(onsets, 0, totalTicks, 0.25);
  ...
  test('tempo bow: 80 → 120 → 80 (continue chaining)', () => {
    ...
    const tempos = getInstructions(mpm, 'tempo', 'global').sort((a, b) => a.date - b.date);
  ...
  test('16-beat linear transition has better accuracy', () => {
    ...
    const onsets = generateOnsets((d) => 80 + 40 * (d / totalTicks), 16);
    const tempos = fitAndGetTempos(onsets, 0, totalTicks, 0.25);
  ...
  test("silentOnsets carrying another performance's timing cannot produce a negative BPM", () => {
    ...
    const silentOnsets: SilentOnset[] = [{ date: 5040, onset: 7700 }];
  ...
  // Superseded by tests/fitting/roundtrip: the 'tempo: ritardando' and 'tempo: accelerando' cases
...
describe('recovering a curve the renderer can draw exactly (#39)', () => {
  ...
  test("the fit is a chord's median onset, not whichever note is listed first", () => {
    ...
    const notes = beats.flatMap((beat, index) =>
      spread.map((offset, voice) => ({
        'xml:id': `n_1_${index}_${voice}`,
        date: beat.date,
        part: 1,
        pitchname: 'g' as const,
        octave: 4 + voice,
        duration: BEAT,
        accidentals: 0,
        'midi.pitch': 67 + 12 * voice,
        'milliseconds.date': beat.onset + offset,
        'milliseconds.date.end': beat.onset + offset + 500,
        velocity: 100,
      })),
    );
