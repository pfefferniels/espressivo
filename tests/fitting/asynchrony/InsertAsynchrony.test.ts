...
import { Mpm, createMpm, getInstructions } from '../../../src/fitting/instructions/index.js';
import { Alignment, type AlignedNote } from '../../../src/fitting/alignment.js';
import { InsertAsynchrony } from '../../../src/fitting/transformers/asynchrony/InsertAsynchrony.js';
...
const offsets = (mpm: Mpm) => getInstructions(mpm, 'asynchrony').map((a) => a.millisecondsOffset);
...
describe('InsertAsynchrony', () => {
  ...
  test('takes the measured shift back off the notes it measured', () => {
    ...
    expect(msm.allNotes.filter((n) => n.part === 1).map((n) => n['milliseconds.date'])).toEqual([
      0, 1000000,
    ]);
  });
  ...
  test('writes nothing when no date pairs up, rather than NaN', () => {
    const { msm, mpm } = run([note('a0', 1, 0, 0), note('b0', 2, 720, 1000000)], 1, 720);
  ...
  test('writes nothing when the part is not in the score at all', () => {
    const { mpm } = run([note('a0', 1, 0, 0), note('a1', 1, 720, 1000000)], 4, 720);
