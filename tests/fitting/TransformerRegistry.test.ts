...
import {
  ...
  isRegistered,
  clearRegistry,
} from '../../src/fitting/transformers/TransformerRegistry.js';
import {
  AbstractTransformer,
...
// below reads.
import { validate } from '../../src/fitting/transformers/Order.js';
...
describe('TransformerRegistry', () => {
  describe('built-in registration', () => {
    ...
    test('getTransformerOrder returns all built-in names in correct relative order', () => {
      ...
      expect(order.indexOf('MakeChoice')).toBeLessThan(
        order.indexOf('ApproximateLogarithmicTempo'),
      );
      expect(order.indexOf('ApproximateLogarithmicTempo')).toBeLessThan(
        order.indexOf('InsertPedal'),
      );
    });
    ...
    test('InsertAsynchrony runs before the physical time it edits is converted', () => {
      ...
      expect(order.indexOf('InsertAsynchrony')).toBeLessThan(
    ...
    test('CompressOrnamentation runs after the defs it rounds are written', () => {
      const order = getTransformerOrder();
      expect(order.indexOf('StylizeOrnamentation')).toBeLessThan(
        order.indexOf('CompressOrnamentation'),
      );
    });
