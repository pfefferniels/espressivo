...
// ---------------------------------------------------------------------------
// DefaultNames – the General MIDI name table
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// getProgramChange – exact matches
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// getProgramChange – fuzzy matches
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// getProgramChange – the distance methods
// ---------------------------------------------------------------------------
describe('InstrumentsDictionary.getProgramChange – distance methods', () => {
    ...
    it.each(methods)('%s should short-circuit on an exact dictionary hit', (_name, method) => {
        // A distance of 0 makes getProgramChange return immediately (Java: line 155-159),
        // so every metric has to agree on names that are dictionary keys.
    ...
    it('should fall back to Normalized Levenshtein for an unknown method id', () => {
        ...
        expect(fallback).toBe(dict.getProgramChange('Klarinett', InstrumentsDictionary.NormalizedLevenshtein));
    });

    it('should default to Normalized Levenshtein when no method is given', () => {
        // Java: getProgramChange(String) delegates with NormalizedLevenshtein, line 93-95
        expect(dict.getProgramChange('Flauto traverso'))
            .toBe(dict.getProgramChange('Flauto traverso', InstrumentsDictionary.NormalizedLevenshtein));
    });
