...
import { Sequence, Track, MidiEvent, ShortMessage, MetaMessage, SysexMessage } from '../../src/midi/MidiTypes.js';
...
describe('Midi', () => {
    ...
    describe('append', () => {
        ...
        it('should not modify the appended Midi', () => {
            const midi = new Midi(480);
            midi.getSequence().createTrack().add(EventMaker.createNoteOn(0, 240, 60, 100)!);
        ...
        it('should create the tracks it is missing', () => {
            ...
            const other = new Midi(480);
            other.getSequence().createTrack().add(EventMaker.createNoteOn(0, 0, 60, 100)!);
            other.getSequence().createTrack().add(EventMaker.createNoteOn(1, 0, 72, 100)!);
        ...
        it('should do nothing for null or empty input', () => {
            const midi = new Midi(480);
            midi.getSequence().createTrack().add(EventMaker.createNoteOn(0, 0, 60, 100)!);
    ...
    describe('print', () => {
        ...
        it('should fall through from program change into the default branch, as Java does', () => {
            ...
            const midi = new Midi(480);
            midi.getSequence().createTrack().add(EventMaker.createProgramChange(0, 0, 42)!);
        ...
        it('should label non-short messages generically', () => {
            const midi = new Midi(480);
            midi.getSequence().createTrack().add(EventMaker.createTempo(0, 120, 0.25)!);
    ...
    describe('readMidiData', () => {
        ...
        it('should restore meta, sysex and channel events in a round trip', () => {
            ...
            track.add(EventMaker.createTempo(0, 120, 0.25)!);
            track.add(new MidiEvent(new SysexMessage(new Uint8Array([0xF0, 0x7E, 0x7F, 0x09, 0x01, 0xF7])), 0));
            ...
            expect(new TextDecoder().decode((trackName.getMessage() as MetaMessage).getData())).toBe('Violin');
        ...
        it('should resolve running status', () => {
            ...
            const data = new Uint8Array([
                0x4D, 0x54, 0x68, 0x64, 0, 0, 0, 6, 0, 0, 0, 1, 0x01, 0xE0,   // MThd, 1 track, ppq 480
                0x4D, 0x54, 0x72, 0x6B, 0, 0, 0, 11,                          // MTrk, 11 bytes
                0x00, 0x90, 0x3C, 0x64,                                       // @0 noteOn 60
                0x60, 0x3E, 0x64,                                             // @96 noteOn 62, running status
        ...
        it('should read a multi-track file', () => {
            const midi = new Midi(480);
            midi.getSequence().createTrack().add(EventMaker.createNoteOn(0, 0, 60, 100)!);
            midi.getSequence().createTrack().add(EventMaker.createNoteOn(1, 0, 72, 100)!);
    ...
    describe('exportMidi – track chunk details', () => {
        ...
        it('should not append a second end of track event', () => {
            ...
            const track = midi.getSequence().createTrack();
            track.add(EventMaker.createNoteOn(0, 0, 60, 100)!);
            track.add(new MidiEvent(new MetaMessage(EventMaker.META_End_of_Track, new Uint8Array(0), 0), 480));

            const parsed = new Midi(midi.exportMidi()!);
            const readTrack = parsed.getSequence().getTracks()[0];
            const endOfTracks = Array.from({ length: readTrack.size() }, (_, i) => readTrack.get(i).getMessage())
        ...
        it('should write format 0 for a single track and format 1 for several', () => {
            ...
            expect(new DataView(singleData.buffer, singleData.byteOffset, singleData.byteLength).getUint16(8)).toBe(0);

            const multi = new Midi(480);
            multi.getSequence().createTrack();
            multi.getSequence().createTrack();
            const multiData = multi.exportMidi()!;
            expect(new DataView(multiData.buffer, multiData.byteOffset, multiData.byteLength).getUint16(8)).toBe(1);
        });
    ...
    describe('exportMsm', () => {
        ...
        it('should fall back to 360 pulses per quarter for a SMPTE sequence', () => {
            ...
            const midi = new Midi(new Sequence(Sequence.SMPTE_30, 80));
            expect(midi.exportMsm()).toBeNull();
            expect(errSpy).toHaveBeenCalledWith('Assuming MIDI time resolution of 360 pulses per quarter.');
